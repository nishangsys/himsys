...
<h1 style="font-size:16px">Total Number of Hours spent by <?php echo $_GET['name']; ?> for <?php echo $_GET['month']; ?>/<?php echo $_GET['year']; ?>  | <span style="background:#1188aa; padding:10px 10px;  text-decoration:none"><a href="my_attend.php?month=
									<?php echo $_GET['month']; ?>&thismonth=<?php echo $row ['thismonth']; ?>&year=<?php echo $_GET ['year']; ?>&name=<?php echo $_GET['name']; ?>" target="new"  class="btn btn-info" style="color:#fff">Print A copy</a></span></h1>
   ...
   <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="example">
                            ...
                            <tbody>
								...
								$month=$_GET['month'];
								$year=$_GET['year'];
								$result= mysql_query("SELECT SUM(used),SUM(shift),COUNT(date),date,name,matric,permit,month,year FROM staff_reg  where month='$month' and year='$year' and name='".$_GET['name']."'  GROUP by id  order by name,id" ) or die (mysql_error());
								$num=1;
  ...
  /* 
   while($ty=mysql_fetch_assoc($R)){
	   echo $totsm=$ty['totalatt'];
                            ...
                            <td style="text-align:center; word-break:break-all; width:80px;"> <?php echo $row ['date']; ?></td>
