...
if(isset($_GET['cust'])){
	...
	$who=$_GET['cust'];
	  ...
	  $select=$conn->query("SELECT * from levels,special,students  where students.id='".$who."' AND special.id=students.dept_id AND levels.id=students.level_id ") or die(mysqli_error($conn));
...
input,select{
 ...
 <form method="post" enctype="multipart/form-data" class="form-horizontal" action="" role="form" >
    ...
    <option value="<?php echo $rows['dept_id'];; ?>"><?php echo $rows['prog_name'];; ?></option></td></tr>
                ...
                <tr><td>Date of Birth</td><td><input type="date" name="dob" value="<?php echo $rows['dob']; ?>" required="required"/></td></tr>
                
              <tr><td>Place of Birth</td><td><input type="text" name="pob" value="<?php echo $rows['pob']; ?>" required="required"/></td></tr>
  ...
  <option value="Female" >Female</option>
               ...
               <tr><td>Level</td><td>
                  <select class="form-control" id="sel1" name="level_id" onBlur="doCalc(this.form)" required>
        <option value="<?php echo $rows['level_id']; ?>">
   <?php echo $rows['levels']; ?>   </option>
...
$date=$_POST['dob'];
$pob=$_POST['pob'];
$stusent_id=$_POST['student_id'];
$fname=$_POST['name'];
$natioanlity=$_POST['nationality'];
...
fname='$fname',
sex='$sex',pob='$pob',dob='$dob',nationality='$natioanlity'  WHERE matricule='$matric' ") or die(mysqli_error($conn)) ;
...
echo '<meta http-equiv="Refresh" content="0; url=../Records/editt.php?cust='.$who.'">';	
