...
session_start();
	...
	if($query->num_rows > 0){
		...
		$headers = array( 'Name', 'Matricule','Sex','Date of Birth','Place of Birth','Telephone Number','Email' );
    	fputcsv($f, $headers, $delimiter);

    	while($row = $query->fetch_array()){
	        $lines = array($row['fname'], $row['matricule'],$row['sex'],$row['dob'],$row['pob'],$row['phone'],
			$row['email']);
