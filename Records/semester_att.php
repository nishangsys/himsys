<form class="form-inline" role="form" method="POST" action="../Records/print_semesteratt.php" target="new">
  <div class="form-group">
    <label for="email">Chose semester:</label>
	...
	students_register.year_id ") or die(mysqli_error($conn));
	$i=1;
		while($bus=$ds->fetch_assoc()){
			?>
                  
                <option value="<?php echo $bus['year_id']; ?>"> <?php echo $bus['year_name']; ?></option>
                <?php } ?>
  ...
  <div class="form-group">
    ...
    <option></option>
     <?php
	$ds=$conn->query("SELECT * FROM semesters where status='1' ") or die(mysqli_error($conn));
	$i=1;
		while($bus=$ds->fetch_assoc()){
			?>
