<form class="form-inline" role="form" method="POST" action="../Records/print_monthlyatt.php" target="new">
  <div class="form-group">
    <label for="email">Month Date:</label>
    <select class="form-control" id="sel1" name="date" required>
    <option></option>
     <?php
	$ds=$conn->query("SELECT * FROM months,years,students_register where students_register.month=months.m_num  AND students_register.year_id=years.id   GROUP BY 
	students_register.month,students_register.year_id ") or die(mysqli_error($conn));
	$i=1;
		while($bus=$ds->fetch_assoc()){
			?>
                  
                <option value="<?php echo $bus['id']; ?>"> <?php echo $bus['m_name']; ?> of <?php echo $bus['year_name']; ?></option>
                <?php } ?>
