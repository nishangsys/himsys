...
input,select{
 ...
 <tr><td>Course Code</td><td><input type="text" name="course" value="<?php echo $course; ?>"  readonly="readonly"/></td></tr>
                ...
                <tr><td>Current Exams Mark</td><td><input type="text" name="ca" value="<?php echo $ca; ?>"  readonly="readonly" style="border:2px solid#f00"/></td></tr>
                
              <tr><td>New Exams Mark</td><td><input type="text" name="nca" required="required" value="<?php echo $ca; ?>" /></td></tr>
		 ...
		 echo $who;		 ?>" required="required"  />
                  ...
                  <tr><td></td><td><button type="submit" name="add" class="btn btn-success btn-lg"   >SAVE Update</button></td></tr>
...
smat='$matric',year_id='$year',fca='$ca',cca='$nca',edited='$user',ip='$localIP',comp='$computer',time='$month',reason='Edit exams mark',course='$course'") or die(mysqli_error($con)) ;

$res=$conn->query("UPDATE resit set c102='$nca',sex='$sem' where roll='$who'") or die(mysqli_error($conn)) ;

//$ress=$conn->query("UPDATE resits set c102='$nca' where fname='$course' and sex='$term' and matricule='$matric'") or die(mysqli_error($conn)) ;
