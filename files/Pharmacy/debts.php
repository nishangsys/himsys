   ...
   <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="example">
                          ...
                          <th style="text-align:center;">Qty Receive</th>
                            ...
                            <tbody>
								...
								$result= mysql_query("select SUM(daily.owed),daily.date,SUM(daily.rec),SUM(daily.price*daily.qty),SUM(daily.qty),SUM(daily.owed),names.name,SUM(daily.rec) from names,daily where daily.owed-daily.rec>'0'
								 and names.id=daily.cust_id  GROUP BY daily.cust_id order by daily.id ASC" ) or die (mysql_error());
								$num=1;
                            ...
                            <?PHP
								...
								$ab= mysql_query("select SUM(daily.owed),SUM(daily.rec),SUM(daily.price*daily.qty),SUM(daily.qty),SUM(daily.owed),names.name,SUM(daily.rec) from names,daily where names.id=daily.cust_id GROUP BY daily.year" ) or die (mysql_error());
								$num=1;
