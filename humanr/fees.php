...
////////////////check from the historique if name exits, then return error else run script			 
	$a=mysql_query("SELECT * FROM historic where matricule='$matrics' and year_id='$ayear' AND amount_paid>0 ") or die(mysql_error());
...
while($bu=$d->fetch_assoc()){
    ...
    <div class="form-group">
      ...
      <div class="col-sm-10">
        <input type="number" class="form-control" id="pwd" name="reg" value="<?php echo $regs; ?>"   >
      </div>
    </div>
...
class='$class1',amountpaid='$paid',amount_paid='$part',expected_amount='$feeamt',balance='$bal',ids='16',date='$date' ,year_id='$ayear',photo='$matric' ,time='$levels',olddebt='$reg' " ) or die(mysqli_error($conn));
 ...
 $daily=$con->query("INSERT INTO daily set cust_id='$matriculex',room='$room',day='$day',staffname='$fname',discount='$reg',amt='$part',date='$date',month='$month',year='$ayear',reason='fees',qty='1',area='1',price='$paid',total='$part',owed='$bal',company='$bank',paidto='$active',
