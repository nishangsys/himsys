...
<h3>Recieving Items from <?php echo $name=$bs['fname'];; ?> 
</h3>
 ...
 <tr><td>Course Code</td><td>    <select required class="form-control" id="sel1" name="item">
        <?php
								...
								$result = $con->query("SELECT * FROM fitems") or die(mysqli_error($con));
...
smat='$matric',year_id='$year',fca='$ca',cca='$nca',edited='$user',ip='$localIP',comp='$computer',time='$month',reason='Items received',course='$item'") or die(mysqli_error($con)) ;
...
$d=$con->query("SELECT * FROM our_items where smat='$mat' and reason='Items received' ") or die(mysqli_error($con));
$i=1;
...
?>
     ...
     <div class='alert alert-success' style="font-weight:bold; font-size:16px">Recieving Items from <?php echo $matric; ?></div>
 <table class="table table-bordered">
    <thead>
      <tr>
      <th>S/N</th>
