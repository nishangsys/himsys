...
$result = mysql_query($one) or die (mysql_error()); 
		...
		$_SESSION['user_level']= $lev;
		
		$query =$con->query("SELECT * FROM sector  WHERE area=".$_SESSION['user_level']) or die(mysqli_error($con));
...
$who=$userRow['link'];
 	...
 	if($query){
			echo '<meta http-equiv="Refresh" content="0; url='.$who.'">';
			
		}
		else
		{
		//$msg = urlencode("Invalid Login. Please try again with correct user email and password. ");
		echo "<script>Invalid Login. Please try again with correct user email and password</script>";
		}
	}
	}
	}
