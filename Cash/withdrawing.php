...
<div class="alert alert-danger">
  <strong>Alert!</strong> Withdrawing from <?php echo $_GET['bank']; ?>
</div>
     <div class="col-sm-6">
      <div class="well">
 <form class="form-horizontal" action="" method="post" name="form">
   ...
   <div class="form-group">
      <label class="control-label col-sm-2" for="pwd">Purpose for Withdrawal:</label>
      <div class="col-sm-10">
        <input type="text" class="form-control" id="pwd" name="purpose"  >
      </div>
    </div>
    ...
    <div class="form-group">
      <div class="col-sm-offset-2 col-sm-10">
        <button type="submit" class="btn btn-danger" name="do" class="btn btn-success" onclick="return confirm('Are you sure about that figure')" class="btn btn-success">Save it</button>
      </div>
    </div>
  </form>
 ...
 else
 {
    echo '<tr bgcolor="White">';
		...
		?>
         ...
         <td><?php echo number_format($bu['formals']); ?></td>
 <td  style="color:#00f"><?php echo number_format($bu['currents']); ?></td>
  <td style="color:#f00"><?php echo number_format($bu['withdrawn']); ?></td>
...
$new=$deposit-$paid;
		 ...
		 $depo=$con->query("INSERT INTO bank_records set num='$accnum',name='$name',formals='$deposit',withdrawn='$paid',date='$date',year='$ayear',newtot='$new'") or die(mysqli_error($con));
		 
		 
		 
 $daily=$con->query("INSERT INTO daily set cust_id='$accnum',room='$accnum',paidto='$active',day='$day',staffname='$name',discount='$paid',amt='',
			rec='$paid',date='$date',month='$month',year='$ayear',reason='bank',qty='1',area='$level',price='$paid',total='$deposit',owed='',
			purpose='Bank Withdrawal',company='$purpose'") or die(mysqli_error($con));
 ...
 $daily12=$con->query("UPDATE our_accounts set amt='$total' where id='$id'") or die(mysqli_error($con));
			...
			echo '<meta http-equiv="Refresh" content="0; url=?withdrawal">';	
}
