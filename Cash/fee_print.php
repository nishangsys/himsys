...
while($bu=$d->fetch_assoc()){
	...
	$d=$con->query("select SUM(rec+bank) as sofar from daily where  staffname='$n' and cust_id='$m' and year='$ay' and reason='fees'") or die(mysqli_error($con));
$a=1;
while($bu=$d->fetch_assoc()){
	$tp=$bu['sofar'];
}
...
Fees for  <?php echo $ay;?> School Year</div>
...
<div style=" float:left; width:500px; height:25px; border-bottom:none; font-size:16px; font-family:Chaparral Pro Light; border-bottom:1PX dashed#000"><i><?php echo number_format($row['owed']) .'&nbsp; FCFA'; ?></i></div>
...
<?php
$d=$con->query("select  * from daily where  staffname='$n' and cust_id='$m' and year='$ay' and reason='fees'") or die(mysqli_error($con));
$a=1;
while($bu=$d->fetch_assoc()){
?>
<tr>
