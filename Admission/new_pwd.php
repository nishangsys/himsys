...
$lev=17;
	...
	$dg=$con->query("SELECT * FROM users where website='$photo' ") or die(mysqli_error($con));
	if(mysqli_num_rows($dg)>0){
		echo $message = '<div class="alert alert-danger">
  <strong>ERROR:</strong>That photo is Already saved in System</div>';
  exit;
	}
	...
	echo $dfj=("UPDATE students set matricule='$matri',cxx1='$pob',cxx2='$dob',sex='$sex' where fname='$name' ") ;exit;
...
$sql_insert2 = $con->query( "INSERT into `teacher`
		    ...
		    VALUES
		    ('$user_name','$pwd','$mname','$final_file','$data[mname]','$name1','$dept'
			)
...
<script language="JavaScript" type="text/javascript" src="../js/jquery.validate.js"></script>
  ...
  $(document).ready(function(){
    ...
    $("#regForm").validate();
     ...
     <BR>
                     	...
                     	<?php /*
		$notes;
		echo $levels; echo $department;*/
	echo	$dfj=("SELECT * FROM users where id='$id' ") ;
		while($rf=$dfj->fetch_assoc()){
			$code=$rf['code'];
			?>
    ...
    <div class="form-group">
      ...
      <div class="col-sm-10">
        <input type="text" class="form-control" id="text" placeholder="Full Name" name="full_name" required value="<?php echo $rf['full_name']; ?>">
      </div>
    </div>
  ...
  <div class="form-group">
     <label class="control-label col-sm-2" for="email">Contact:</label>
      <div class="col-sm-10">
        <input type="text" class="form-control" id="email" name="tel">
      </div>
    </div>
    ...
    <div class="form-group">
     <label class="control-label col-sm-2" for="email"></label>
      <div class="col-sm-10">
        <input type="hidden" class="form-control" id="email" name="dept" value="<?php echo $_GET['dept']; ?>">
        <?php } ?>
      </div>
    </div>
