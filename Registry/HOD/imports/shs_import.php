			...
			if(strtolower($fileinfo['extension']) == 'csv'){
				...
				if($_FILES['file']['size'] > 0){
					...
					while(($impData = fgetcsv($file, 1000, ',')) !== FALSE){
						...
						$check =$conn->query( "SELECT * FROM my_records   WHERE matric='$impData[1]' and code='$code' and sem='$sem'   and ayear='$ayear'") or die(mysqli_error($conn));
						$exist=$check->num_rows;;
						///check if Exams amrks Exists already
						
					 $check_ca= $dbcon->query("SELECT * FROM my_records   WHERE matric='$impData[1]' and code='$code' and sem='$sem'   and ayear='$ayear' and exam!=''  ") or die(mysqli_error($dbcon));	
							
		        $ca_exist=$check_ca->num_rows;
				$rounded=round($impData[2]);
					 if($ca_exist>0){
					   
						$do=$dbcon->query("UPDATE  my_records SET   ca='".$rounded."'   WHERE code='$code' AND matric='".$impData[1]."' AND sem='$sem' AND ayear='$ayear'    ") or die(mysqli_error($dbcon));
				   }
						else  if($exist>0){
