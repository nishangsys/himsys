...
?>
 ...
 <?php
   ...
   if($_GET['sch_id']==2 || empty($_GET['sch_id']) ){
		     ...
		     $ok=$conn->query("DELETE FROM my_records where  ayear='$ayear' and code='$code' AND matric like '%MS%' ") or die(mysqli_error($conn));
	   ...
	   $ok=$conn->query("DELETE FROM my_records where dept='$dept' and ayear='$ayear' and code='$code' and exam IS NULL ") or die(mysqli_error($conn));
	   $oks=$conn->query("DELETE FROM my_records where dept='$dept' and ayear='$ayear' and code='$code' and exam='NULL' ") or die(mysqli_error($conn));
...
<div class="alert alert-info">
  <strong> <span style="color:#f00">Uploading  CA marks for <?php echo $dept_name; ?> LEVEL <?php echo $level; ?> this <?php echo $ayear=$_GET['ayear']; ?></span> </strong>
</div>
  ...
  <table class="table table-bordered table-striped">
				...
				<tbody>
					...
					if($_GET['sch_id']==2){
						$sql = "SELECT * FROM my_records where sem='".$semester."'
					 AND ayear='".$ayear."' 
					AND code='".$code."'  ";
					}
					else if(empty($_GET['sch_id'])){
