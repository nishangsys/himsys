...
include  '../includes/dbc.php';
	...
	$d=$dbcon->query("SELECT *  from  my_marks where dept='$dept' and ayear='".$ayear."'  and sem='$sem' AND levels='$level' and ca!='' GROUP BY matric") or die(mysqli_error($dbcon));
	$page=1;
	while($row=$d->fetch_assoc()){
		
		
	$dhj=$dbcon->query("SELECT *  from courses where  matricule='".$row['matric']."' ") or die(mysqli_error($dbcon));
	
	while($rl=$dhj->fetch_assoc()){
		$name=$rl['fname'];
		$matric=$rl['matricule'];
	}
...
<?php } ?></div>
	...
	$ds=$dbcon->query("SELECT *  from  my_marks where matric='".$matric."' and ayear='".$ayear."'  and sem='$sem' AND ca!='' order by code ") or die(mysqli_error($dbcon));
