...
$year=$_POST['grade'];
	...
	////get Current page number and Name 
			 $cl=$dbcon->query("SELECT * FROM  my_marks where levels='$level' and sem='$sem' and dept='$prog' and ayear='$year' AND ca!='' GROUP BY matric  order by matric ") or die(mysqli_error($dbcon));
			 $a=1;
...
?>
	<page size="A4" layout="landscape"  >
        ...
        <div class="main" style="height:691px; border:; outline:#FFF; font-size:11px; ";
		  >
           ...
           <h2 style="margin-top:20px"> <?PHP echo $month; ?>  Ca and Exam Marks for  <?php echo $prog;  ?>( Level <?php echo $level;  ?>) FOR <?php  echo $year; ?> School Year</h2>
           </div>
    
    <div style=" clear:both" ></div>
    
    <table class="table table-bordered" style="margin-top:10px; line-height:1.5">
    <thead>
      <tr>
        <th>S/N</th>
        <th>Name</th>
	...
	while($rows=$cls->fetch_assoc()){
		...
		<!--
		<h5 style="  -webkit-transform: rotate(-90deg); 
    -moz-transform: rotate(-90deg); 
    filter: progid:DXImageTransform.Microsoft.BasicImage(rotation=3); text-align:left; 
	margin-left:-150px ;padding:80px 50px; position:relative; clear:both; margin-:30px; margin-top:50px  ">
  ...
  <div class="boxes" style="border-right:none; width:20%">Ca</div><div class="boxes" style="border-right:none;width:20%"> Exm </div><div class="boxes" style="border-right:none;width:25%"> Totl</div><div class="boxes" style="border-right:none; width:25%"> Grde</div><br />
	...
	while($rowl=$cl->fetch_assoc()){
      ...
      <tr>
		...
		$add=$dbcon->query("SELECT fname FROM courses where matricule='".$rowl['matric']."' ") or die(mysqli_error($dbcon));
