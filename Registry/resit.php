...
<div style="   width:100%; float:left; text-align:center;  
  ...
  border:1px DASHED#000; font-size:16px; padding:5px 0px; text-transform:uppercase  "> p.o box 77 bokoko ,buea</div></div>
    ...
    <div style="   width:100%; float:left; text-align:center;  
  border:1px DASHED#000; font-size:16px;padding:5px 0px; font-weight:bold;   ">RESITS <?PHP echo $row['db1']; ?>  INDIVIDUAL RESULTS SLIP</div>
    <!--left box------->
   <div class="left_box">
...
<?php } ?></div>
	...
	$ds=$dbcon->query("SELECT *  from  my_marks where matric='".$row['matricule']."' and ayear='".$row['db1']."'  and sem='3' ") or die(mysqli_error($dbcon));
   ...
   <td style="font-size:10px"><?php
	...
	/////Credit Earned
	 ...
	 $uu=$dbcon->query("UPDATE  my_marks set valid='1'   where levels='".$row['levels']."' and   code='".$rows['code']."' and matric ='".$row['matricule']."' ") or die(mysqli_error($dbcon));
			}
			else {
    ...
    <div class="footer_box">
			...
			/////Credit Earned
		    $as=$dbcon->query("SELECT * FROM  my_marks where   matric='".$row['matricule']."' and ayear='".$row['db1']."' and sem='3'  and valid='1' ") or die(mysqli_error($dbcon));
