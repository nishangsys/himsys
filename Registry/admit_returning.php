...
while($bu=$d->fetch_assoc()){
	...
	///check exists matricuile for current schol year
	$assd=$conn->query("SELECT * from courses where matricule='".$mats."' and db1='$ayear' order by roll desc limit 1 ") or die(mysqli_error($conn));
	  $exists=$assd->num_rows;
	  
	  
	  $ass=$conn->query("SELECT * from courses where matricule='".$mats."'  order by roll desc limit 1 ") or die(mysqli_error($conn));
	  
	  if($exists>0){
		  echo "<script>alert('$y_name has already been registered this $ayear . Please try again later')</script>";
	  }
...
$assm=$con->query("SELECT * from special where certi='".$bs['departmet']."'  ") or die(mysqli_error($con));
	while ($bsm=$assm->fetch_assoc()){
		 $sch=$bsm['sch'];
		 $dept=$bsm['school'];
...
input,select{
 ...
 ?>" /></td></tr>
    ...
    <option value="<?php echo $bs['sex']; ?>"><?php echo $bs['sex']; ?></option>
  ...
  <tr><td>Dept I .D </td><td><input type="text" name="ids" value="<?php
		 echo $id;		 ?>" required="required" /></td></tr>
...
matricule='$matric' and db1='$db1' ") or die(mysqli_error($conn)) ;
	...
	/* $ats12=$conn->query("UPDATE  courses  set  
sex='$sex',c110='$ids',cxx1='$POB',cxx2='$DOB' ,cxx6='$sch',cxx4='Cameroonian' WHERE matricule='$matric'  ") or die(mysqli_error($conn)) ;*/
