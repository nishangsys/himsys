...
while($bu=$d->fetch_assoc()){
	...
	while ($rows=$select->fetch_assoc()){
		...
		$dept=$rows['departmet'];
		  ...
		  $ch_exist=$conn->query("SELECT * from courses  where fname='$name' and departmet='$dept' and db1='$ayear'  and matricule!='' ") or die(mysqli_error($conn));
	...
	*/
	 ...
	 $ass=$con->query("SELECT * from special where certi='".$rows['departmet']."'  ") or die(mysqli_error($con));
	while ($bs=$ass->fetch_assoc()){
		 $sch_ab=$bs['begc'];
		 $dept_ab=$bs['code'];
		 $num=$bs['stat'];
...
input,select{
 ...
 <form method="post" enctype="multipart/form-data" class="form-horizontal" action="" role="form" >
     ...
     <table class="table table-bordered">
              ...
              <tr><td>Program</td><td><input type="text" name="class" required="required" value="<?php echo $rows['departmet'];; ?>" readonly="readonly" /></td></tr>
...
matricule='$matricule' WHERE roll='".$who."' ") or die(mysqli_error($conn)) ;
...
echo '<meta http-equiv="Refresh" content="0; url=?buib_matricule">';	
