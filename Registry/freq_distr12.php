...
?>
	...
	while($rowl=$cl->fetch_assoc()){
      ...
      <tr>
        ...
        <td ><?php  echo $rowl['code']; ?></td>
         ...
         <td >100%</td>
          <td > <?php
		  /////Total Number of students enrolled that is from the courses tbl
		$cls=$dbcon->query("SELECT * FROM courses where levels='$level'  and departmet='$prog' and db1='$year' GROUP BY  matricule ") or die(mysqli_error($dbcon));
	echo  $num_enrolled=$cls->num_rows;
		?></td>
        
        <td > <?php
		////Total Number of students that sat for this Courses
		$cls=$dbcon->query("SELECT * FROM  my_marks where levels='$level'  and dept='$prog' and ayear='$year' and sem='$sem' and code='".$rowl['code']."'  GROUP BY  matric ") or die(mysqli_error($dbcon));
	...
	echo  $fail=$cls->num_rows;
		...
		//////Percentage Pass: 
		$as= (($pass/$num_sat)*100);
		echo $per_pass=number_format($as,1);
		?> </td>
	...
	while($ks=$lo->fetch_assoc()){
     ...
     <td> <?php
		  /////Total Number of students enrolled that is from the courses tbl
		$cls=$dbcon->query("SELECT SUM(credit) as totcredit FROM subject where level='$level'  and department='$prog' and sem='$sem'  ") or die(mysqli_error($dbcon));
	    while($rw=$cls->fetch_assoc()){
			echo $totcredit=$rw['totcredit'];
		}
		?></td>
         <td ></td>
         <td >100%</td>
		  ...
		  /////Total Number of students enrolled that is from the courses tbl
		$cls=$dbcon->query("SELECT * FROM courses where levels='$level'  and departmet='$prog' and db1='$year' GROUP BY  matricule ") or die(mysqli_error($dbcon));
	  $num_enrolled=$cls->num_rows ;
	  echo $num_enrolled*$a;
		?></td>
        
        <td > <?php
		////Total Number of students that sat for this Courses
		$cls=$dbcon->query("SELECT * FROM  my_marks where levels='$level'  and dept='$prog' and ayear='$year' and sem='$sem'    ") or die(mysqli_error($dbcon));
	echo  $num_sat=$cls->num_rows;
		?></td>
        
        <td > <?php
		//////Pass: Number of students with marks greater than 49
		$cls=$dbcon->query("SELECT * FROM  my_marks where levels='$level'  and dept='$prog' and ayear='$year' and sem='$sem'   and (ca+exam>49)  GROUP BY  matric,code ") or die(mysqli_error($dbcon));
	echo  $pass=$cls->num_rows;
		?></td>
         ...
         <td > <?php
		//////Fail: Number of students with marks less than 49
		$cls=$dbcon->query("SELECT * FROM  my_marks where levels='$level'  and dept='$prog' and ayear='$year' and sem='$sem'   and (ca+exam<50)  GROUP BY  matric,code ") or die(mysqli_error($dbcon));
	echo  $fail=$cls->num_rows;
		?></td>
	...
	while($ks=$lo->fetch_assoc()){
        ...
        <td style="padding:0px 5px"  >
		<?php  $as=$dbcon->query("SELECT * FROM  my_marks where levels='$level'  and dept='$prog' and ayear='$year' and sem='$sem'   and (ca+exam)>=$lower and (ca+exam)<=$upper  	 ") or die(mysqli_error($dbcon));
		echo $ffg=$as->num_rows; ?>
		</td>
       <?php } //// close 	while($ks=$lo->fetch_assoc()){
        ?>
...
<div class="f1">CR: Candidate Registered</div>
