...
$c=$dbcon->query("SELECT * FROM courses,my_Registry WHERE    courses.departmet='$deptss'  and courses.db1='$ayear' and courses.levels='$level'   and courses.matricule=my_Registry.matric and  courses.db1=my_Registry.ayear and  my_marks.matric!='' GROUP BY  my_marks.matric ") or die(mysqli_error($dbcon));
	$page=1;
	$page_num=$c->num_rows;
while($row=$c->fetch_assoc()){
 $dept=$row['departmet'];
...
$cl=$dbcon->query("SELECT * FROM `my_Registry` WHERE matric='".$row['matricule']."'   GROUP BY ayear") or die(mysqli_error($dbcon));

echo  $h_many=$cl->num_rows;
...
?>
<h3>BUILDING <span style="color:#00F"><?PHP echo $row['fname'];  ?> </span> TRANSCRIPT PLEASE TAKE A CUP OF TEA WHILE IT LOADS</h3>
	<page size="A4" layout="landscape">
        <div class="main" style="height: <?php  if($cur_pnum==$h_many){
			echo "760px; border:1px solid#000; border-bottom:1px solid#000; ";
