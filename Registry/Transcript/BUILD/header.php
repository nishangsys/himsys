...
/////get start year	
$df=$dbcon->query("SELECT * FROM students WHERE   matricule='".$matric."' and dept_id='$dept' order by year_id ASC LIMIT 1") or die(mysqli_error($dbcon));
...
/////get end year	
$df=$dbcon->query("SELECT * FROM students WHERE   matricule='".$matric."' and dept_id='$dept' order by year_id DESC LIMIT 1") or die(mysqli_error($dbcon));
...
$as=$conn->query("SELECT * FROM degrees where dept_id='". $ydept."'   ") or die(mysqli_error($conn));
...
?>
   ...
   $firstname = substr($fullname, 0, strpos($fullname, ' '));
	...
	/////if you have attained all credits		  
			echo	$row['cxx5'];
						   ?></span>
                  <br />
                   Date :  <span style="font-size:14px; font-weight:bold"> August <?php if($graduate==1){
					   echo $end;
				   }
				   else {
				   }
