	 ...
	 $uu=$conn->query("UPDATE  my_marks set valid='0'   where level_id='".$row['level_id']."' and  year_id='".$row['year_id']."' and code='".$ans['code']."' and matric='".ltrim($row['matricule'])."' ") or die(mysqli_error($conn));
		  ...
		  while ($bs=$as->fetch_assoc()){
			...
			echo $grade=$bs['grade'];  
			 ...
			 $uu=$conn->query("UPDATE  my_marks set grade='$grade' where level_id='".$row['level_id']."' and  year_id='".$row['year_id']."' and code='".$ans['code']."' and matric ='".ltrim($row['matricule'])."' and sem='2' ") or die(mysqli_error($conn));
			 
			  $uu=$conn->query("UPDATE  my_marks set graded='$grade' where level_id='".$row['level_id']."' and  year_id='".$row['year_id']."' and code='".$ans['code']."' and matric ='".ltrim($row['matricule'])."' and sem='2' ") or die(mysqli_error($conn));
		  ...
		  $as=$conn->query("SELECT SUM(cv) as attempts FROM my_marks where matric='".ltrim($row['matricule'])."' and sem='2' 
		  and year_id='".$row['year_id']."' ") or die(mysqli_error($conn));
		...
		/////////check if records exists else insert
		  $asl=$conn->query("SELECT * FROM my_stats where matric='".ltrim($row['matricule'])."' and sem='2' and year_id='".$row['year_id']."' ") or die(mysqli_error($conn));
		  $exist=$asl->num_rows;
		  ...
		  if($exist>=1){
			 ...
			 $uu=$conn->query("UPDATE my_stats set attempt='$credit_attempted' where  matric='".ltrim($row['matricule'])."' and sem='2' and year_id='".$row['year_id']."' ") or die(mysqli_error($conn));
			}
			
			/////if not exixt insert
			else {
			$uu=$conn->query("INSERT INTO my_stats SET attempt='$credit_attempted',matric='".ltrim($row['matricule'])."', sem='2' ,year_id='".$row['year_id']."' ") or die(mysqli_error($conn));
		...
		/////////check if records exists else insert
		  ...
		  if($exist>=1){
			 ...
			 $uu=$conn->query("UPDATE my_stats set earn='$credit_earned' where  matric='".ltrim($row['matricule'])."' and sem='2' and year_id='".$row['year_id']."' ") or die(mysqli_error($conn));
			}
			
			/////if not exixt insert
			else {
			$uu=$conn->query("INSERT INTO my_stats  SET earn='$credit_earned',matric='".ltrim($row['matricule'])."', sem='2' ,year_id='".$row['year_id']."' ") or die(mysqli_error($conn));
		...
		/////////check if records exists else insert
		  ...
		  if($exist>=1){
			 ...
			 $uu=$conn->query("UPDATE my_stats set gpa='$second_semseter_gpa' where  matric='".ltrim($row['matricule'])."' and sem='2' and year_id='".$row['year_id']."' ") or die(mysqli_error($conn));
			}
			
			/////if not exixt insert
			else {
			$uu=$conn->query("INSERT INTO my_stats   SET gpa='$second_semseter_gpa',matric='".ltrim($row['matricule'])."', sem='2' ,year_id='".$row['year_id']."' ") or die(mysqli_error($conn));
