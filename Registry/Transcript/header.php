...
/////get start year	
$df=$dbcon->query("SELECT * FROM years,students WHERE   students.matricule='".$matric."' and students.year_id=years.id AND  students.dept_id='$dept_id' order by students.year_id ASC LIMIT 1") or die(mysqli_error($dbcon));
...
/////get end year	
$df=$dbcon->query("SELECT * FROM years,students WHERE   students.matricule='".$matric."' and students.year_id=years.id AND  students.dept_id='$dept_id' order by students.year_id DESC LIMIT 1") or die(mysqli_error($dbcon));
...
$as=$conn->query("SELECT * FROM degrees where dept_id='".$dept_id."'   ") or die(mysqli_error($conn));
...
?>
     ...
     Student's No:<BR /> <span style="font-size:14px; margin-top:-10px; font-weight:bold"><?php echo ltrim($row['matricule']); ?> </span>
         </div>
   ...
   $firstname = substr($fullname, 0, strpos($fullname, ' '));
     $lastname = substr($fullname, strpos($fullname, ' '), strlen($fullname));
        ...
        <span style="font-size:14px; font-weight:bold"> <?php echo $row['dob']; ?></span>        
        </div>
        ...
        <span style="font-size:14px; font-weight:bold"><?php echo $row['pob']; ?></span>  
        </div>
        ...
        <p style="margin-top:-20px;  font-size:12px; line-height:1.5"> This Transcript is not valid without the signature of the Registrar and Enbossed seal of the school</p>
          ...
          <div class="box1" style="border-top:0px;  width:375px; height:80px">
                ...
                <p style="margin-top:0px;  font-size:12px; line-height:1.5; margin-top:-20px"> Department :  <span style="font-size:14px; font-weight:bold"> <?php echo $row['prog_name']; ?></span> <br />
                
                 Degree Proposed :  <span style="font-size:13px; font-weight:bold"> <?php echo $degree_conferred; ?></span> 
                 <br />
                  Degree Conferred :  <span style="font-size:13px; font-weight:bold"> <?php if($graduate==1){
				   ...
				   ?></span>
                  <br />
                   Date :  <span style="font-size:14px; font-weight:bold"> July <?php if($graduate==1){
					   echo $end;
				   }
				   else {
				   }
