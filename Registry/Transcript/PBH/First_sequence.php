 ...
 <?PHP
		  ...
		  $ad=$conn->query("SELECT * FROM  my_marks where matric='".$stripped."' and levels='".$row['levels']."' and  ayear='".$row['db1']."' and sem='1'  GROUP BY roll order by code ") or die(mysqli_error($conn));
	...
	////Update Registry with total/100 for avg	  	
			 $uu=$conn->query("UPDATE  my_marks set earned='".$  my_marks."' where  matric='".$stripped."' and sem='1' and ayear='".$row['db1']."' and code='".$ans['code']."'  ") or die(mysqli_error($conn));
	////if mark is greater than 50 then its a pass mks or else he is 
	if($  my_marks>=50){
		////PASS WITH VALID 1
			  	
			 $uu=$conn->query("UPDATE  my_marks set valid='1' where  matric='".$stripped."' and sem='1' and ayear='".$row['db1']."' and code='".$ans['code']."'  ") or die(mysqli_error($conn));
	}
	else {
		////FAIL WITH VALID 0
		
			 $uu=$conn->query("UPDATE  my_marks set valid='0' where  matric='".$stripped."' and sem='1' and ayear='".$row['db1']."' and code='".$ans['code']."'  ") or die(mysqli_error($conn));
	}
 ...
 $asm=$conn->query("SELECT * FROM  my_marks where matric='".$stripped."' and sem='1' and ayear='".$row['db1']."'   ") or die(mysqli_error($conn));
 /////Number of Courses written in SEM
 $num_of_course=$asm->num_rows;
 
 /////Sum total of all 1 sem courses on 100
		   $as=$conn->query("SELECT SUM(earned) as attempts FROM  my_marks where matric='".$stripped."' and sem='1' and ayear='".$row['db1']."'   ") or die(mysqli_error($conn));
