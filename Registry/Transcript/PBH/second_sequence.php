 ...
 <?PHP
	...
	////Update Registry with total/100 for avg	  	
			 $uu=$conn->query("UPDATE  my_marks set earned='".$  my_marks."' where  matric='".$stripped."' and sem='2' and ayear='".$row['db1']."' and code='".$ans['code']."'  ") or die(mysqli_error($conn));
	////if mark is greater than 50 then its a pass mks or else he is 
	if($  my_marks>=50){
		////PASS WITH VALID 1
			  	
			 $uu=$conn->query("UPDATE  my_marks set valid='1' where  matric='".$stripped."' and sem='2' and ayear='".$row['db1']."' and code='".$ans['code']."'  ") or die(mysqli_error($conn));
	}
	else {
		////FAIL WITH VALID 0
		
			 $uu=$conn->query("UPDATE  my_marks set valid='0' where  matric='".$stripped."' and sem='2' and ayear='".$row['db1']."' and code='".$ans['code']."'  ") or die(mysqli_error($conn));
	} 
		   ?>
            </div>
