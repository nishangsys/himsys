          ...
          <div class="semester_box" style=" margin-top:-370px; height:365px; width:356px">
 <?PHP
				 $stripped = str_replace(' ', '', ltrim($row['matricule']));
				
		  $ad=$conn->query("SELECT * FROM  my_marks where matric='".$stripped."' and levels='".$row['levels']."' and  ayear='".$row['db1']."' and sem='3'  GROUP BY roll order by code ") or die(mysqli_error($conn));
		  ...
		  ?>  
             ...
             <div  class="patition" style="border:none;; line-height:none; width:196PX;height:15px ; padding:5px 0px; font-size:9px; text-align:left; "> &nbsp;
               <?php
 ...
 $asm=$conn->query("SELECT * FROM  my_marks where matric='".$stripped."' and sem='3' and ayear='".$row['db1']."'   ") or die(mysqli_error($conn));
 /////Number of Courses written in SEM
 $num_of_course=$asm->num_rows;
 
 /////Sum total of all 1 sem courses on 100
		   $as=$conn->query("SELECT SUM(earned) as attempts FROM  my_marks where matric='".$stripped."' and sem='3' and ayear='".$row['db1']."'   ") or die(mysqli_error($conn));
