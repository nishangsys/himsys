...
$year=$_POST['ayear'];
	...
	////get Current page number and Name 
			 $cl=$dbcon->query("SELECT * FROM   my_marks where level_id='$level' and sem='$sem' and dept_id='$prog' and year_id='$year'  GROUP BY matric  order by matric ") or die(mysqli_error($dbcon));
			 $a=1;
...
?>
    ...
    <thead>
      <tr>
		...
		$cls=$dbcon->query("SELECT * FROM   my_marks where level_id='$level'
		and sem='$sem' and dept_id='$prog' and year_id='$year' and exam is NOT NULL 
		GROUP BY code  order by code ") or die(mysqli_error($dbcon));
...
">
   ...
   <?php
		...
		$add=$dbcon->query("SELECT title FROM subjects where code='".$rows['code']."'
		 order by id DESC limit 1  ") or die(mysqli_error($dbcon));
  ...
  <div class="boxes" style="border-right:none; width:20%">Ca</div><div class="boxes" style="border-right:none;width:20%"> Exm </div><div class="boxes" style="border-right:none;width:25%"> Totl</div><div class="boxes" style="border-right:none; width:25%"> Grde</div><br />
	...
	while($rowl=$cl->fetch_assoc()){
      ...
      <tr>
        ...
        <?php
		$cls=$dbcon->query("SELECT * FROM   my_marks where level_id='$level' and sem='$sem' and dept_id='$prog' and year_id='$year' and exam IS NOT NULL GROUP BY code  order by code ") or die(mysqli_error($dbcon));
...
<!----Select from marks where tbl where matric equals to this and courses code is equals to this --------->
 <?php
		$kl=$dbcon->query("SELECT * FROM   my_marks where level_id='$level' and sem='$sem' and matric='".$rowl['matric']."' and  year_id='$year' and code='".$rows['code']."' ") or die(mysqli_error($dbcon));
