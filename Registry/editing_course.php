...
*/
 ...
 $ats=$conn->query("SELECT * FROM subject  WHERE  
sem='$sem' AND level='$level' AND department='$dept' and subject='$code' ") or die(mysqli_error($conn)) ;
if($ats->num_rows>0){
...
echo '<meta http-equiv="Refresh" content="0; url=?edit_course">';
 ...
 $ats=$conn->query("INSERT INTO subject  set  
title='$title',credit='$cv',status='$status',sem='$sem',level='$level',department='$dept',subject='$code'  ") or die(mysqli_error($conn)) ;
