...
function checkAvailability() {
 ...
 <form class="form-horizontal" action="" method="post" name="form">
  ...
  <input name="username" type="text" id="username" class="demoInputBox" onBlur="checkAvailability()" style="width:60%"><span id="user-availability-status"></span>    
...
if(isset($_POST['do'])){
 ...
 $daily=$con->query("INSERT INTO daily set cust_id='0',room='$dept',paidto='$active',day='$day',staffname='$name',discount='$reg',amt='',
			rec='$paid',date='$date',month='$month',year='$year_id',reason='$reason',qty='1',area='$level',price='$paid',total='$paid',owed='',
			purpose='$reason',company='cash'") or die(mysqli_error($con));
			echo "<script>alert('Records Successfull')</script>";
			echo '<meta http-equiv="Refresh" content="0; url=?record_others">';	
}
