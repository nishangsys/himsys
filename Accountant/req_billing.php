...
$rid=$_GET['yid'];	
	...
	?>
        ...
        <div class="col-sm-12" style="padding:10PX 0PX; background:#006; border:1PX solid#fff; font-weight:bold; color:#ff0">
         <a href=javascript:popcontact('change_sales.php?tabs=<?php echo $table; ?>&area=0') style="color:#fff; text-decoration:blink text-align:center; font-weight:bold;">
    ...
    Validate 
            ...
            <?PHP
			$a = $con->query("SELECT SUM(qty),SUM(total),price,product,category,ids,id,price,qty,total FROM requisitions where tab='".$_GET['tabs']."' and status='0' and qty>0 GROUP BY category,product order by product ") or die(mysqli_error($con));
			$i=1;
			?>
  ...
  <a href="index.php?our_staff=<?php echo $name; ?>&dep=<?php echo $dept; ?>&id=<?php echo $table; ?>&adds=<?php echo $rows['id']; ?>&name=<?php echo $name; ?>">
 ...
 <a href="index.php?our_staff=<?php echo $name; ?>&dep=<?php echo $dept; ?>&id=<?php echo $table; ?>&reduce=<?php echo $rows['id']; ?>&name=<?php echo $name; ?>">
...
<?php } ?>  
 ...
 <!--------TOTAL---->   
   <div class="col-sm-12" style="background:#003; padding:0px 0px; text-align:CENTER; border:1px solid#fff"> 
    <span style="background:#090; padding:15px 40px; float:left; font-size:18px; color:#fff; font-weight:bold ">  Total</span>
     ...
     <span style="background:#090; padding:15px 20px; float:right; font-size:18px; color:#fff; font-weight:bold ">  
      <?PHP
      	$a = $con->query("SELECT SUM(price*qty) as totbill FROM requisitions where tab='".$_GET['tabs']."' and status='0'  and requisitions.qty>0 GROUP BY tab") or die(mysqli_error($con));
