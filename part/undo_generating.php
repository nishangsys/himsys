...
@session_start();
$month=date('m');	
$query 	= mysql_query("SELECT * FROM `payslips` WHERE month='$month' ORDER BY id ASC"); // Select from the table
...
if(isset($_POST['SubmitUpdate'])) {
		...
		$month=date('m');
			$qry =mysql_query( "DELETE FROM payslips where month='$month' ") or die(mysql_error()); // loop the mysql_query values to avoid more server loding time
...
<div class="as_wrapper">
	<h1 class="h1"><a href=""> YOU ARE ABOUT TO UNDO PAYSLIP FOR <span style="color:#f00"><?php echo date('F Y'); ?> </span> </a></h1>
  ...
  <td><input type="text" name="gross<?php echo $i; ?>"   value="<?php echo (($salary+$resp+$others+$overtime)-$deductions); ?>"   class="input" style="width:100px;background:#ee4a40; color:#fff"   /></td>
            ...
            <tr>
                ...
                <input type="submit" name="SubmitUpdate" value="UNDO <?php echo date('F'); ?> payslip generation"  onclick="return confirm('Are you Sure YOU want to undo salaries for this month for all saved data will be lost ')" style="background:#F00; color:#fff; padding:10px 10px" /></td>
            </tr>
            </table>
            </form>
        <?php
