...
function doCalc(form) {
  ...
  form.youwillpay.value = ((parseInt(form.exp.value) - parseInt(form.oldpa.value))+parseInt(form.paidu.value));
    ...
    <table>
		 ...
		 while($ro=mysql_fetch_assoc($update)){
			 $overtime=$ro['time'];
               ...
               <tr><td> Suppose to Depart On</td><td><input type="text" name="suppo" value="<?php echo $row['duedate']; ?>" onBlur="doCalc(this.form)" readonly="readonly"   /></td>
               <td>
...
while($rows=mysql_fetch_assoc($rusn)){
	  ...
	  //if days used more than 1 and the time is above the overtime, give it add 2 days
	   ...
	   //if days used more than 1 and the time is below the overtime, give it add 1 days	 
		   echo $not=abs($rows['DAYS'])+1;
				 ...
				 ?>"    required="required" style="width:300px;" onBlur="doCalc(this.form)"/></td><td >Amt Paid </td><td><input type="text" value="<?PHP echo number_format($row['paid']); ?> Frs"  style="color:#f00; text-decoration:blink" /></td></tr>

                 <tr><td> Amount Consumed In Lodging</td><td><input type="text" name="consumed"  style="width:300px;color:red;" onBlur="doCalc(this.form)" required="required" readonly  /></td><td>
  ...
  <tr><td> Others Bills</td><td><input type="text" name="otherbills"  style="width:300px;color:red;" onBlur="doCalc(this.form)"
            required="required" readonly value="<?php
			 ...
			 ?>"   /></td></tr>
                 ...
                 <tr><td> Total Bills Owed</td><td><input type="text" name="exp"  style="width:300px; background:#39C;color:white;" onBlur="doCalc(this.form)" required="required"  /></td></tr>
  ...
  <tr>
                  <td>Amount to be refunded</td><td><input type="text" name="paidu" style="width:300px; background:#9F3" onBlur="doCalc(this.form)" required="required"
				   ...
				   $first=(($row['paid']-($not*$row['cost']))-$row['otherbal']-$ovtm); 
				  if($first<=0){
					  echo "0";
				  }
				  else {
...
<tr>
                  ...
                  required="required"  value="<?php $first=(($row['paid']-($not*$row['cost'])+$ovtm)-$row['otherbal']); 
				    $ovtm;
 ...
 <?php if(isset($_POST['saves'])){
	...
	$diff=$paidold-$paidtou;
	 ...
	 $update_finances=mysql_query("UPDATE finances set otherbal='$bal', status='2',level='$date' where yourid='".$id."'") or die(mysql_error()); 
		//daily records
	   ...
	   $insert_report=mysql_query("INSERT INTO reports set checkin='$ind',yourid='".$id."',date='$date',month='$month',year='$year',days_uesd='$days_consumed',withus='$bal',paidin='$paid',paidout='".$paidtou."',cout='$date',name='$name',room='$room'") or die(mysql_error()) ;
