...
<h1>CONTRACT WORKERS SALARY VOUCHER FOR  20<?PHP $y=date('y')-1;  echo $y;?> - <?php
 ...
 <span style="float:right">Department:Contract Workers</span>
	...
	$month=$_GET['month'];
	$dept=$_GET['dept'];
	$cust="SELECT * from staffs,payslips where payslips.month='$month'  and payslips.year='$today' and payslips.matric=staffs.matric and staffs.paytax='1' ";
 ...
 else
 {
   ...
   <td><?php echo  number_format($gross-$alltaxes);?></td>
    ...
    <td>TOTAL</td>
    <td><?PHP
    $S="SELECT SUM(payslips.salary) as totsal from payslips,staffs where payslips.month='$month'  and payslips.matric=staffs.matric and staffs.paytax='1' ";
    ...
    $S="SELECT SUM(payslips.resp),SUM(payslips.rents),SUM(payslips.senior),SUM(payslips.trans),SUM(payslips.leaves),SUM(payslips.overtime)  from payslips,staffs where payslips.month='$month' and payslips.matric=staffs.matric and staffs.paytax='1'";
	$run=mysql_query($S) or die (mysql_error());
	while($r=mysql_fetch_assoc($run)){
    ...
    <td><?PHP
    $S="SELECT SUM(payslips.others) as totothers from staffs,payslips where month='$month' and payslips.dept='$dept' and payslips.matric=staffs.matric and staffs.paytax='1' ";
	$run=mysql_query($S) or die (mysql_error());
	while($r=mysql_fetch_assoc($run)){
    ...
    $S="SELECT SUM(payslips.cnps) as totcnps from payslips,staffs where payslips.month='$month'  and payslips.matric=staffs.matric and staffs.paytax='1'";
	$run=mysql_query($S) or die (mysql_error());
	while($r=mysql_fetch_assoc($run)){
		 $totcnps=$r['totcnps'];
    ...
    $S="SELECT SUM(payslips.irpp),SUM(payslips.ccf),SUM(payslips.crtv),SUM(payslips.nhlf)  from payslips,staffs where payslips.month='$month'  and payslips.matric=staffs.matric  and staffs.paytax='1' ";
	$run=mysql_query($S) or die (mysql_error());
	while($r=mysql_fetch_assoc($run)){
    ...
    $S="SELECT SUM(payslips.others_exp),SUM(payslips.pre_paid)  from payslips,staffs where payslips.month='$month'  and payslips.matric=staffs.matric  and staffs.paytax='1'";
	$run=mysql_query($S) or die (mysql_error());
	while($r=mysql_fetch_assoc($run)){
