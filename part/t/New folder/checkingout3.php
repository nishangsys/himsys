...
function doCalc(form) {
  form.exp.value = ((parseInt(form.addeddays.value) * parseInt(form.cost.value))) + parseInt(form.bala.value);
...
?>
    ...
    <table>
          ...
          <tr><td></td><td><input type="hidden" name="id" value="<?php echo  $row['yourid']; ?>" style="width:300px" readonly="readonly" /></td></tr>
             ...
             <tr><td></td><td><input type="hidden" name="addeddays" value="<?php echo  $row['newadd']; ?>"  /></td></tr>
...
while($rows=mysql_fetch_assoc($rusn)){
	 ...
	 elseif($diff<=$not) {
		  echo  $diff=$not;
                 ...
                 <tr><td> Amount you will Pay You</td><td><input type="text" name="exp"  style="width:300px; background:red;color:white;" onBlur="doCalc(this.form)" required="required"  /></td></tr>

                <tr><td>Amount Paid</td><td><input type="text" name="paid" style="width:300px" onBlur="doCalc(this.form)" required="required"  />
                </td></tr>
 ...
 <?php if(isset($_POST['saves'])){
		...
		//daily records
		$update_client=mysql_query("UPDATE ourclients set bal='".$bal."' where yourid='".$id."'") or die(mysql_error());
		//update room status to empty
		$update_finances=mysql_query("UPDATE rooms set status='1' where num='".$room."'") or die(mysql_error());
