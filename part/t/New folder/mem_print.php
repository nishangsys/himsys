...
if(isset($_GET['id'])){
	$who=$_GET['id'];
	  $select="SELECT * from member,members_times,daily
	   where member.matric=members_times.matricule and members_times.matricule=daily.time  and members_times.matricule='$who'  
	...
	while ($rows=mysql_fetch_assoc($run)){
	 ...
	 $select1="SELECT owed as totowed from members_times where matricule='$who' ";
	$run1=mysql_query($select1) or die(mysql_error());
	while ($rowss=mysql_fetch_assoc($run1)){
		$totowed=($rowss['totowed']);
...
font-size:18px; ">
N<SUP>0</SUP> 00<?PHP echo $row['id']; ;?>
...
<div style=" float:left; width:190px; height:25px;font-size:17px;"> As Payments for .</div>


<div style=" float:left; width:400px;border-bottom:1px solid #000; height:25px;font-size:17px;"> 


<div style=" float:left; width:400px;margin-top:3px;">
<i><?php echo $row['disc'];?> (<?php echo $row['inter'];?>)</i>
</div>
...
font-size:13px; ">
<div style=" float:left; width:170px; height:25px;font-size:17px;"> Amt Paid today</div>
...
<?php echo number_format($row['rec']) ;?> <i>frs C. F. A</i>
</div>
...
<div style=" float:left; width:300px;margin-top:3px;">
<?php echo number_format($totowed);?>.FCFA
</div>
</div>
...
<?php
  $sel="SELECT * FROM daily where time='$who' order by id DESC LIMIT 4";
...
<div class="t">
<?php
$sel="SELECT SUM(rec) as total FROM daily where time='$who'";
 $ones=mysql_query($sel) or die(mysql_error()); 
