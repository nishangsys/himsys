...
<div class="right">
  ...
  ?>
<h1 > You are about to add bonuses to  <span style="color:#F00"> <?php echo $myname=$row['name']; ?> </span> </span></h1> 
  ...
  <tr><td></td><td><input type="HIDDEN" name="matric"  value="<?php echo $row['matric'] ?>" /></td></tr>
                ...
                <tr><td>Position</td><td><input type="text" name="post"  style="width:80%" value="<?php echo $row['age'] ?>" readonly /></td></tr>
                                ...
                                <tr><td></td><td><input type="HIDDEN" name="one"  value="<?php echo $row['one'] ?>" /></td></tr>
                                                                <tr><td>Responsibilty </td><td><input type="text" name="response"  style="width:60%"  value="<?php echo $row['response'] ?>" /></td></tr>
                                ...
                                <tr><td>Should Pay Tax</td><td><select name="chose" style="width:60%" required="required">
...
$_POST = array_map("ucwords", $_POST);
	 ...
	 */
	 ...
	 $image=mysql_query("UPDATE staffs SET response='$resp',research='$research',rents='$rents',senior='$senior',
	 trans='$trans',leaves='$leave',others='$others',
	 overtime='$overtime',paytax='$pay' where  id='$ids' ") or die(mysql_error());

	 
	 $image=mysql_query("UPDATE payslips SET leaves='$leave' where  matric='$matric' ") or die(mysql_error());
	 
	  $image=mysql_query("UPDATE voucher SET leaves='$leave' where  matric='$matric' ") or die(mysql_error());
		
				echo "<script>alert('You have successfully add bonus to $myname. Thank You')</script>";
					echo '<meta http-equiv="Refresh" content="0; url=staffpage.php?my_bonus">';
