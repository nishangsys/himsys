...
FROM members_times where id='".$ro['id']."'");
while($rows=mysql_fetch_assoc($rusn)){
		...
		if($not<0){
			...
			echo "<span style='color:#f00'>Only Today Left</span>";
		}
		else {
			echo "<span style='color:#f00'>Expired Membership</span>";
		}
   ...
   <h1>All Registered Members</h1>
    ...
    <td><?php
		...
		$rusn=mysql_query("SELECT DATEDIFF(CURDATE(),STR_TO_DATE(duedate, '%m/%d/%Y')) AS DAYS
FROM members_times where id='".$row['id']."'");
while($rows=mysql_fetch_assoc($rusn)){
	     $not=($rows['DAYS']);
		if($not<0){
			echo abs($not)." &nbsp; Days";
