<?php
/**
Simple all_departments Create, Read, Update and Delete (CRUD) using php mysql by asif18.com, for more tutorials visit: http://www.asif18.com
for this tutorial visit: http://asif18.com/20/php/all_departments-insert-update-using-php-mysql-delete-all_departments-records-using-checkboxes-in-php/
**/
include '../dbc.php'; // include the database and server connection file
$query 	= mysql_query("SELECT * FROM `all_departments` ORDER BY id ASC"); // Select from the table
...
if(isset($_POST['submit'])) {
	...
	$amt = $_POST['total'];
	if($amt > 0) {
		$qry = "INSERT INTO all_departments(code,name) VALUES "; // Split the mysql_query
		for($i=1; $i<=$amt; $i++) {
...
// all_departments delete case using checkboxes
if(isset($_POST['SubmitDelete'])) {
	$_POST = array_map("ucwords", $_POST);
	$amt = $_POST['total']; // Get the total rows
	for($i=1; $i<=$amt; $i++) {
		$delete = mysql_query("DELETE FROM all_departments WHERE id = '".$_POST["delete$i"]."'"); // Run the delete query inside for loop
	}
...
// all_departments update case
if(isset($_POST['SubmitUpdate'])) {
	$_POST = array_map("ucwords", $_POST);
	$amt = $_POST['total']; // Get the total rows
	for($i=1; $i<=$amt; $i++) {
		$update = mysql_query("UPDATE `all_departments` SET `code` = '".$_POST["amt$i"]."',
		 name = '".$_POST["name$i"]."' WHERE `id` = '".$_POST["id$i"]."'"); // Run the Mysql update query inside for loop
	}
...
<meta name="keywords" content="all_departments insert in php, all_departments crud using PHP MySql, all_departments insert update delete using php mysql"/>
<meta name="description" content="all_departments insert update delete CRUD using PHP MySql. A simple way to insert, update and delete all_departments values at a time using PHP MySql"/>
<style>
.as_wrapper{
	margin:0 auto;
	width:800px;
...
<div class="as_wrapper">
	<h1 class="h1"><a href="">Add/Edit and Delete a Department</a></h1>
    <p style="line-height:1.5">To add all_departments note the Following<br>
    ...
    ?>
        ...
        ?>
            ...
            <tr>
                ...
                <td><input type="text"name="name<?php echo $i; ?>" style="width:230px" />               
                </td>
        ...
        elseif(isset($_GET['update'])) {
           ...
           <tr style="background:#1188aa; color:#fff">
            ...
            <tr>
                ...
                <td><input type="text" name="amt<?php echo $i; ?>" value="<?php echo $row['code']; ?>" class="input" style="width:80px"  /></td>
