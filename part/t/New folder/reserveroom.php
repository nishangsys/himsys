...
<div class="right">    
 ...
 while($r=mysql_fetch_assoc($run12)){
	 ...
	 $idd=$r['id'];
		    ...
		    $exits=mysql_query("SELECT * from reserves where day='$day' and room='$room' AND category='$cate' ")or die(mysql_error()) ;
			$coun=mysql_num_rows($exits);
			if($coun>0){
				
			echo "<script>alert('ERROR.Room ".$room." has been reserved for ".$day." .Try again')</script>";
		echo '<meta http-equiv="Refresh" content="0; url=../reception/frontpage.php?see_allreserves">';
