...
if($not<0){
 ...
 while($r=mysql_fetch_assoc($run12)){
	 $room=$r['num'];
	 $block=$r['id'];
	 echo "<a href='frontpage.php?report=".$block."' style='color:#C06; text-decoration:blink' target='_blank'>Report</a>";
	 
 }
  }
  else {
		$today=date('m/d/Y');
 $run123=mysql_query("SELECT * from reserves where day='$today'") or die (mysql_error());
 if(mysql_num_rows($run123)>0){
 while($rr=mysql_fetch_assoc($run123)){
	$rooms=$rr['room'];
	  $blocks=$rr['block'];
	 $cust=mysql_query("UPDATE rooms set status='3' , who='".$row['cust_id']."' where num='$rooms'  and floor='$blocks' and status='1'") ;
...
<td>
<a href="../reception/frontpage.php?changedate=<?php echo $row['client_id'] ?>" style="color:#f00" onclick="return confirm('Are you Sure you want to change <?php echo $row['stu_name']; ?> date')">Change Date !</a></td>
