...
$_POST = array_map("ucwords", $_POST);
	 ...
	 */
	 ...
	 $image=mysql_query("INSERT INTO loans set  loan='$other_deducts',reason='$reasons',matric='$matric' ,name='$myname',month='$month',year='$year',dept='$dept',date='$date' ") or die(mysql_error());
   ...
   <table>
    <?php
   $df=mysql_query("SELECT * FROM loans where name='$myname' and reason!='' order by id DESC") or die(mysql_error());
   $i=1;
   ?>
...
if($month==$month1 and $year==$year1){
	$delete=mysql_query("DELETE FROM loans where id='$id'") or die(mysql_error());
		...
		echo '<meta http-equiv="Refresh" content="0; url=staffpage.php?adding_sanctions='.$_GET['adding_sanctions'].'">';
