<?php
/**
Simple bonus_cat Create, Read, Update and Delete (CRUD) using php mysql by asif18.com, for more tutorials visit: http://www.asif18.com
for this tutorial visit: http://asif18.com/20/php/bonus_cat-insert-update-using-php-mysql-delete-bonus_cat-records-using-checkboxes-in-php/
**/
include '../dbc.php'; // include the database and server connection file
$query 	= mysql_query("SELECT * FROM `bonus_cat` ORDER BY id ASC"); // Select from the table
...
if(isset($_POST['submit'])) {
	...
	$amt = $_POST['total'];
	if($amt > 0) {
		$qry = "INSERT INTO bonus_cat(amt,name) VALUES "; // Split the mysql_query
		for($i=1; $i<=$amt; $i++) {
...
// bonus_cat delete case using checkboxes
if(isset($_POST['SubmitDelete'])) {
	$_POST = array_map("ucwords", $_POST);
	$amt = $_POST['total']; // Get the total rows
	for($i=1; $i<=$amt; $i++) {
		$delete = mysql_query("DELETE FROM bonus_cat WHERE id = '".$_POST["delete$i"]."'"); // Run the delete query inside for loop
	}
...
// bonus_cat update case
if(isset($_POST['SubmitUpdate'])) {
	$_POST = array_map("ucwords", $_POST);
	$amt = $_POST['total']; // Get the total rows
	for($i=1; $i<=$amt; $i++) {
		$update = mysql_query("UPDATE `bonus_cat` SET `amt` = '".$_POST["amt$i"]."', name = '".$_POST["catego$i"]."' WHERE `id` = '".$_POST["id$i"]."'"); // Run the Mysql update query inside for loop
	}
...
<meta name="keywords" content="bonus_cat insert in php, bonus_cat crud using PHP MySql, bonus_cat insert update delete using php mysql"/>
<meta name="description" content="bonus_cat insert update delete CRUD using PHP MySql. A simple way to insert, update and delete bonus_cat values at a time using PHP MySql"/>
<style>
.as_wrapper{
	margin:0 auto;
	width:800px;
...
<div class="as_wrapper">
	<h1 class="h1"><a href="">Add/Edit and Delete a Bonus Category</a></h1>
    <p style="line-height:1.5">To add bonus_cat note the Following<br>
   ...
   <span style="color:#f00"> 3. Please no CFA or Frs is allowed after figures</span><br>
    ...
    ?>
        ...
        <table align="center">
        <tr>
            <td style="background:#1188aa; color:#fff; padding:10px 10px">Number of bonus_cat you want to add</td>
        ...
        ?>
            ...
            <tr>
                <td><?php echo $i; ?></td>
                <td><input type="text" name="name<?php echo $i; ?>" style="width:200px"  />
               </td>                
