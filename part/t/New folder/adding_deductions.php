  ...
  <tr><td></td><td><input type="HIDDEN" name="matric"  value="<?php echo $row['matric'] ?>" /></td></tr>
                ...
                <tr><td>Position</td><td><input type="text" name="post"  style="width:80%" value="<?php echo $row['age'] ?>" readonly /></td></tr>
                                ...
                                <tr><td></td><td><input type="HIDDEN" name="one"  value="<?php echo $row['one'] ?>" /></td></tr>
                                                                <tr><td>Advance Salary </td><td><input type="text" name="advance"  style="width:60%"  value="<?php echo $row['advance'] ?>" /></td></tr>


                                <tr><td>Other Deductions</td><td><input type="text" name="other_deducts"  style="width:60%"  value="<?php echo $row['others'] ?>" /></td></tr>
                                
                                 <tr><td>Reasons for deductions</td><td><input type="text" name="reasons"  style="width:90%"   /></td></tr>
...
$_POST = array_map("ucwords", $_POST);
	 ...
	 */
	 ...
	 $b=mysql_query("SELECT * FROM payslips WHERE month='$month'  and  matric='$matric' and name='$myname' ") or die(mysql_error());
	 if(mysql_num_rows($b)<1){
		 echo "<script>alert('ERROR. You have not generate payslip for this month')</script>";
	 }

else {
	 	
	 $image=mysql_query("UPDATE payslips set  pre_paid='$advance',others_exp='$other_deducts',reasons='$reasons' where  matric='$matric' and name='$myname' ") or die(mysql_error());
