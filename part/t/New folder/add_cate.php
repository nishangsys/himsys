<?php
/**
Simple all_categories Create, Read, Update and Delete (CRUD) using php mysql by asif18.com, for more tutorials visit: http://www.asif18.com
for this tutorial visit: http://asif18.com/20/php/all_categories-insert-update-using-php-mysql-delete-all_categories-records-using-checkboxes-in-php/
**/
include '../dbc.php'; // include the database and server connection file
$query 	= mysql_query("SELECT * FROM `all_all_categories` ORDER BY id ASC"); // Select from the table
...
if(isset($_POST['submit'])) {
	...
	$amt = $_POST['total'];
	if($amt > 0) {
		$qry = "INSERT INTO all_categories(amt,name) VALUES "; // Split the mysql_query
		for($i=1; $i<=$amt; $i++) {
			$qry .= "('".$_POST["amt$i"]."','".$_POST["name$i"]."'), "; // loop the mysql_query values to avoid more server loding time
		}
...
// all_categories delete case using checkboxes
if(isset($_POST['SubmitDelete'])) {
	$_POST = array_map("ucwords", $_POST);
	$amt = $_POST['total']; // Get the total rows
	for($i=1; $i<=$amt; $i++) {
		$delete = mysql_query("DELETE FROM all_categories WHERE id = '".$_POST["delete$i"]."'"); // Run the delete query inside for loop
	}
...
// all_categories update case
if(isset($_POST['SubmitUpdate'])) {
	$_POST = array_map("ucwords", $_POST);
	$amt = $_POST['total']; // Get the total rows
	for($i=1; $i<=$amt; $i++) {
		$update = mysql_query("UPDATE `all_categories` SET `amt` = '".$_POST["amt$i"]."', name = '".$_POST["catego$i"]."' WHERE `id` = '".$_POST["id$i"]."'"); // Run the Mysql update query inside for loop
	}
...
<meta name="keywords" content="all_categories insert in php, all_categories crud using PHP MySql, all_categories insert update delete using php mysql"/>
<meta name="description" content="all_categories insert update delete CRUD using PHP MySql. A simple way to insert, update and delete all_categories values at a time using PHP MySql"/>
<style>
.as_wrapper{
	margin:0 auto;
	width:800px;
...
<div class="as_wrapper">
    ...
    <p style="line-height:1.5">To add all_categories note the Following<br>
   ...
   <span style="color:#f00"> 3. Please no CFA or Frs is allowed after figures</span><br>
    ...
    ?>
        ...
        <table align="center">
        <tr>
            <td style="background:#1188aa; color:#fff; padding:10px 10px">Number of all_categories you want to add</td>
        ...
        ?>
            ...
            <tr>
                <td><?php echo $i; ?></td>
                <td><input type="text" name="catego<?php echo $i; ?>" style="width:80px"  />
               </td>                
                    
            
                <td><input type="text"name="amt<?php echo $i; ?>" style="width:120px" />               
                </td>
        ...
        elseif(isset($_GET['update'])) {
           ...
           <tr style="background:#1188aa; color:#fff">
            ...
            <tr>
                ...
                <td><input type="text" name="catego<?php echo $i; ?>" value="<?php echo $row['name']; ?>" class="input"  /></td>
