...
<h1 style="background:#00B1BA; color:#fff">Clients That have nothing before Taking a Room</h1>
    ...
    <TD><?php    	//SELECT DATEDIFF(CURDATE(),STR_TO_DATE(mysql field, 'date-format-------%d-%m-%Y'))
...
while($rows=mysql_fetch_assoc($rusn)){
	 ...
	 elseif($diff<=$not) {
		  echo  $diff=$not;
				 ...
				 ?> Days</TD>
    <td>
        <a href=javascript:popcontact('payingovtzero.php?zerout=<?php echo $row['id'] ?>') style="color:#f00">Check me out!</a></td>
