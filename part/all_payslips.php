...
$mat=$_GET['yourown'];
	...
	$year=$_GET['year'];;
	$d=mysql_query("SELECT * FROM payslips,voucher where payslips.month='$month' and voucher.month='$month' AND payslips.name=voucher.name and payslips.year='$year' GROUP BY payslips.name") or die(mysql_error());
...
<div style="  height:AUTO; width:100%; float:left; text-align:center; background:#333; color:#FFF; 
  border:1px DASHED#000;text-transform:uppercase; font-size:18px; font-weight:bold  "> salary voucher FOR FOR BIAKA UNIVERSITY INSTITUTE OF BUEA(HIMS) </div>
