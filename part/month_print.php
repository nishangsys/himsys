...
<h1>SALARY VOUCHER FOR  20<?PHP $y=date('y')-1;  echo $y;?> - <?php
 ...
 echo date('Y'); ?>  SCHOOL YEAR</h1>
  ...
  <div style="width:90%; margin:auto; padding:10px 10px; font-weight:bold; font-weight:bold; text-align:center;" ><span style="float:left">Month: <?php echo $_GET['thism']; ; ?></span>
 <span style="float:right">Department: <?php echo $_GET['dept']; ?></span>
	...
	$month=$_GET['month'];
	$dept=$_GET['dept'];
	$cust="SELECT * from staffs,payslips where payslips.month='$month' and payslips.dept='$dept' and payslips.year='$today' and payslips.matric=staffs.idcard  GROUP BY staffs.idcard order by payslips.name";
 ...
 else
 {
    ...
    <td style="text-align:left; padding:0px 10px" S><?php echo $row['matric']; ?></td>
   ...
   <td><?php echo  number_format($gross-$alltaxes);?></td>
    ...
    <td>TOTAL</td>
    <td><?PHP
    $S="SELECT SUM(payslips.salary) as totsal from payslips,staffs where payslips.month='$month' and payslips.dept='$dept' and payslips.matric=staffs.idcard ";
    ...
    $S="SELECT SUM(payslips.resp),SUM(payslips.rents),SUM(payslips.senior),SUM(payslips.trans),SUM(payslips.leaves),SUM(payslips.overtime)  from payslips,staffs where payslips.month='$month' and payslips.dept='$dept' and payslips.matric=staffs.idcard ";
	$run=mysql_query($S) or die (mysql_error());
	while($r=mysql_fetch_assoc($run)){
    ...
    $S="SELECT SUM(payslips.cnps) as totcnps from payslips,staffs where payslips.month='$month' and payslips.dept='$dept'  and payslips.matric=staffs.idcard ";
	$run=mysql_query($S) or die (mysql_error());
	while($r=mysql_fetch_assoc($run)){
		 $totcnps=$r['totcnps'];
    ...
    $S="SELECT SUM(payslips.irpp),SUM(payslips.ccf),SUM(payslips.crtv),SUM(payslips.nhlf)  from payslips,staffs where payslips.month='$month' and payslips.dept='$dept'  and payslips.matric=staffs.idcard  ";
	$run=mysql_query($S) or die (mysql_error());
	while($r=mysql_fetch_assoc($run)){
    ...
    $S="SELECT SUM(payslips.others_exp),SUM(payslips.pre_paid)  from payslips,staffs where payslips.month='$month' and payslips.dept='$dept'  and payslips.matric=staffs.idcard  ";
	$run=mysql_query($S) or die (mysql_error());
	while($r=mysql_fetch_assoc($run)){
