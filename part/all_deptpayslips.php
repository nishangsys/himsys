...
$mat=$_GET['yourown'];
	...
	$year=$_GET['year'];;
	$d=mysql_query("SELECT * FROM payslips,voucher where payslips.month='$month' and voucher.month='$month' AND payslips.name=voucher.name and payslips.year='$year' AND payslips.dept!='POLICLINIC' GROUP BY payslips.name") or die(mysql_error());
...
<div style="  height:AUTO; width:100%; float:left; text-align:center; background:#333; color:#FFF; 
  border:1px DASHED#000;text-transform:uppercase; font-size:18px; font-weight:bold  "> salary voucher  FOR BIAKA UNIVERSITY INSTITUTE OF BUEA(HIMS)  </div>
...
<!--others---->
 ...
 <div style="width:5%; height:20px; float:left; border:1px dashed#000; padding:2px 5px;text-align:center; font-weight:bold; ">7</div>
    ...
    <div style="width:46%; height:20px; float:left; text-align:LEFT; border:1px dashed#000; padding:2px 5px; ">Others  <?php
	 $reasonss=$row['reasons'];
	if(empty($row['others_exp'])){
		echo "";
	}
	else {
		echo "($reasonss)";
...
<div style="width:97%; margin-top:5PX; height:20px; float:left; border:1px dashed#000; background:#E5F5F9; padding:10px 5px;text-align:LEFT;  ">
                 ...
                 <?php 
                   ...
                   <span style="margin-left:20PX;">EMPLOYEE'S SIGNATURE</span><span style="float:right; margin-right:20PX"> EMPLOYER'S SIGNATURE</span></div>
