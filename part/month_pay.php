    ...
    <a class="btn btn-primary" href="<?PHP echo $_SERVER['_SELF']; ?>?register_staff"  style="text-decoration:none; color:#fff; padding:5px 10px">Click Here To Add </a>
			...
			$months=$_POST['month'];
				...
				$status=$_POST['status'];
								...
								$result= mysql_query("select * from payslips where month='$months' and year='$year' and paid='$status'" ) or die (mysql_error());
								$i=1;
             ...
             <td style="text-align:center; word-break:break-all; width:10px;"> <?php echo $i++; ?></td>
                            ...
                            <td style="text-align:center; word-break:break-all; width:130px; background:#9FC"> <?php echo $row ['salary']+$row['resp']+$row['research']+$row['rents']+$row['senior']+$row['leaves']+$row['trans']+$row['overtime']+$row['others']; ?></td>
                               ...
                               <a href="../salary/all_paidslips.php?all=<?php echo $row ['matric']; ?>&month=
							 ...
							 ?>       
                                     <a href="../salary/salary.php?our_salaries&paid=<?php echo $row ['id']; ?>" class="btn btn-success" style="color:#fff" >  Paid</a>
                                    <?php 
							 }
							 else {
									?>
   <a href="../salary/salary.php?our_salaries&npaid=<?php echo $row ['id']; ?>" class="btn btn-danger" style="color:#fff"> Not Paid</a>
                                    <?php } ?>
