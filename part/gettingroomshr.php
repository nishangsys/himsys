...
$query_parent = mysql_query("SELECT * FROM rooms where num='" . $_GET['cat']. "' limit 1") or die("Query failed: ".mysql_error());
...
<?php
 $cat=$_GET['cat'];
 $v=mysql_query("SELECT * from rooms where num='$cat'");
 while($r=mysql_fetch_assoc($v)){
	  $a=$r['cateogry'];
 }
 $rt=$a;
 if($rt='rest room' or $rt='Rest Room' or $rt='REST ROOM' or $rt='Rest'){
