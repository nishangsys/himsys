...
if(isset($_POST['SubmitUpdate'])) {
	...
	if($amt > 0) {
		...
		for($i=1; $i<=$amt; $i++) {
	  $check=mysql_query("DELETE from tobe_paid where month='$month' and year='$year' and dept!='POLICLINIC'") or die(mysql_error());
...
leaves='".$_POST["leaves$i"]."',trans='".$_POST["trans$i"]."',senior='".$_POST["senior$i"]."',pre_paid='".$_POST["loan$i"]."',other_exp='".$_POST["deducts$i"]."' ") or die(mysql_error()); // loop the mysql_query values to avoid more server loding time
		...
		echo '<meta http-equiv="Refresh" content="1; url=generating.php?voucher='.$month.'&name='.$tm.'">';
...
<div class="as_wrapper">
	<h1 class="h1"><a href="">Tax Declaration for<span style="color:#f00">  <?php echo $_GET['name']; ?> <?php echo date('Y'); ?> </span> </a></h1>
            ...
            <tr>
				...
				echo $ccf=round($irpp*0.1);
				  ...
				  ?>" style="width:60px" class="input" readonly /></td>
                   ...
                   <td><input type="text" name="others<?php echo $i; ?>" style="width:60px" value="<?php 
	...
	$sk=mysql_query("SELECT * from loans where reason=''  and name='".$row['name']."'  and loan>0  ") or die(mysql_error());
				 while($atk=mysql_fetch_assoc($sk)){
		...
		else if($startmonth<$months && $syear==$year){ 
		echo 0;
	}
	
	else if($months<$startmonth and $syear==$year){
		echo 0;
	}
		...
		echo  $topay;
            ...
            <tr>
                ...
                <input type="submit" name="SubmitUpdate" value="DECLARE TAXES FOR <?php echo $_GET['name']; ?> <?php echo date('Y'); ?> " style="background:#1188aa; color:#fff; padding:10px 10px" /></td>
            </tr>
            </table>
            </form>
        <?php
