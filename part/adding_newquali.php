<?php
/**
Simple quali Create, Read, Update and Delete (CRUD) using php mysql by asif18.com, for more tutorials visit: http://www.asif18.com
for this tutorial visit: http://asif18.com/20/php/quali-insert-update-using-php-mysql-delete-quali-records-using-checkboxes-in-php/
**/
include '../dbc.php'; // include the database and server connection file
$query 	= mysql_query("SELECT * FROM `quali` ORDER BY id ASC"); // Select from the table
...
if(isset($_POST['submit'])) {
	...
	$amt = $_POST['total'];
	if($amt > 0) {
		$qry = "INSERT INTO quali(name) VALUES "; // Split the mysql_query
		for($i=1; $i<=$amt; $i++) {
...
// quali delete case using checkboxes
if(isset($_POST['SubmitDelete'])) {
	$_POST = array_map("ucwords", $_POST);
	$amt = $_POST['total']; // Get the total rows
	for($i=1; $i<=$amt; $i++) {
		$delete = mysql_query("DELETE FROM quali WHERE id = '".$_POST["delete$i"]."'"); // Run the delete query inside for loop
	}
...
if(isset($_POST['SubmitUpdate'])) {
	...
	$amt = $_POST['total']; // Get the total rows
	for($i=1; $i<=$amt; $i++) {
		$update = mysql_query("UPDATE `quali` SET 
...
<meta name="keywords" content="quali insert in php, quali crud using PHP MySql, quali insert update delete using php mysql"/>
<meta name="description" content="quali insert update delete CRUD using PHP MySql. A simple way to insert, update and delete quali values at a time using PHP MySql"/>
<style>
.as_wrapper{
	margin:0 auto;
	width:800px;
...
<div class="as_wrapper">
	<h1 class="h1"><a href="">Add/Edit and Delete a New Qualification</a></h1>
    <p style="line-height:1.5">To add quali note the Following<br>
