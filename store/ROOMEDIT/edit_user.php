...
for($i=0;$i<$usersCount;$i++) {
	$toal[$i]=$_POST["price"][$i]*$_POST["qty"][$i];
	$rt=mysql_query("UPDATE rooms_products set product='" . $_POST["product"][$i] . "', price='" . $_POST["price"][$i] . "',total='" . $toal[$i]. "',
 quatity='" . $_POST["qty"][$i] . "' WHERE pro_id='" . $_POST["pro_id"][$i] . "'") or die(mysql_error());
...
$result = mysql_query("SELECT * FROM rooms_products WHERE pro_id='" . $_POST["users"][$i] . "'") or die(mysql_error());
$rt=1;
...
<td>SN</td>
<td></td>
<td>Product</td><td>Room</td><td>Selling Price</td><td>Quantity in stock</td>
</tr>
