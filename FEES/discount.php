...
<?php
$ayear;
$d=$conn->query("SELECT * FROM historic where amountpaid>0 GROUP BY ayear order by roll ") or die(mysqli_error($conn));
while($bn=$d->fetch_assoc()){
?>

<a href="?discount&year=<?php echo $bn['ayear']; ?>">
<div class="col-sm-3" style="border:2px solid#f00; color:#F03">
         
            <h4>  <?php echo $bn['ayear']; ?> Discount</h4>
...
<script type="text/javascript" charset="utf-8" language="javascript" src="js/DT_bootstrap.js"></script>
   ...
   <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="example">
                          ...
                          <th style="text-align:center;">Program</th>
                            ...
                            <tbody>
								...
								$result= $conn->query("select  * from historic WHERE amountpaid>0  and year_id='$ac' order by roll DESC" ) or die (mysqli_error($conn));
								$num=1;
                            ...
                            <td style="text-align:center; word-break:break-all; width:20px;"> <?php echo $num++; ?></td>
								...
								<td style="text-align:center; word-break:break-all; width:250px;"> <?php echo $row ['amountpaid']; ?></td>
                                    ...
                                    <td style="text-align:center; word-break:break-all; width:80px;"> <?php echo $row ['ayear']; ?></td>
                                             ...
                                             <td style="text-align:center; word-break:break-all; width:80px; color:#f00; font-weight:b"> <?php echo $row ['amountpaid']; ?></td>
   ...
   <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="example">
						...
						$result= $conn->query("select  SUM(amountpaid) from historic WHERE amountpaid>0 and year_id='$ac' " ) or die (mysqli_error($conn));
								$num=1;
				while ($row= $result->fetch_assoc() ){
								$id=$row['id'];
								?>
                        ...
                        <td style="text-align:center; word-break:break-all; width:130px; background:#060; font-weight:bold; color:#fff"> <?php echo number_format($row ['SUM(amountpaid))']); ?></td>
            ...
            <td style="text-align:center; word-break:break-all; width:130px; background:#F00; color:#fff;">
             <?php echo number_format($row ['SUM(amountpaid)']); ?>					
        </td>     
