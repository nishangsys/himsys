...
?>
    ...
    <span style="color:#ff0; font-weight:bold"> <?php echo $dept; ?> LEVEL <?php echo $level; ?></span> SCHOOL FEES REPORTS FOR <span style="color:#ff0; font-weight:bold"><?PHP echo $ayears; ?> ACADEMIC YEAR
  
       | <a href="do_perlevel.php?dept=<?php echo $dept; ?>&level=<?php echo $level; ?>&year_id=<?PHP echo $ayears; ?>" target="_new"> <button type="submit" class="btn btn-warning" name="doLogin" class="btn btn-danger">Download a Copy</button></a>
      </span>
      </div>             
...
<script type="text/javascript" charset="utf-8" language="javascript" src="js/DT_bootstrap.js"></script>
   ...
   <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="example">
                          ...
                          <th style="text-align:center;">Program</th>
                            ...
                            <tbody>
								<?php
							echo $ayears;
								$result= mysql_query("select  * from historic where year_id='$ayears' and amountpaid='$dept' and class='$level' order by student_name" ) or die (mysql_error());
								$num=1;
   ...
   <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="example">
                            ...
                            <tbody>
								...
								$result= mysql_query("select  SUM(expected_amount),SUM(amount_paid),SUM(balance) from historic where year_id='$ayears' and amountpaid='$dept' and class='$level' GROUP BY class" ) or die (mysql_error());
								$num=1;
								while ($row= mysql_fetch_array ($result) ){
								$id=$row['id'];
								?>
