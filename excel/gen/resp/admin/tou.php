...
<body>
	...
	$run=mysql_query($cust) or die (mysql_error());
	 ...
	 ?>
     <h1 style="background:#1188aa; color:#fff; padding:20px 0px; text-align:center; color:#fff"><?php echo $old; ?> Room Supply Record for Today</h1>
   ...
   <div style="width:50px; height:30px; text-align:center; border:1px solid#000; float:left">S/N</div>
    ...
    <div style="width:250px; height:30px; text-align:center; border:1px solid#000; float:left">Cost of Supplies</div>
    ...
    <div style="width:250px; background:#f00; color:#ff0; height:30px; text-align:center; border:1px solid#000; float:left"><?php
	 $old=$_GET['id'];
	  $today=date('d-m-Y');
	  $area=$_GET['area'];
	$cust=mysql_query("SELECT SUM(amt) from daily where date='$today' and paidto='$old' AND area='$area' ") or die(mysql_error());
	while($row=mysql_fetch_assoc($cust)){
		echo number_format($row['SUM(amt)']);
	}
