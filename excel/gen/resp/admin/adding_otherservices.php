<?php
/**
Simple other_services Create, Read, Update and Delete (CRUD) using php mysql by asif18.com, for more tutorials visit: http://www.asif18.com
for this tutorial visit: http://asif18.com/20/php/other_services-insert-update-using-php-mysql-delete-other_services-records-using-checkboxes-in-php/
**/
include '../dbc.php'; // include the database and server connection file
$query 	= mysql_query("SELECT * FROM `other_services` ORDER BY id ASC"); // Select from the table
...
if(isset($_POST['submit'])) {
	...
	$amt = $_POST['total'];
	if($amt > 0) {
		$qry = "INSERT INTO other_services(amt,facilities,cat,lastprice) VALUES "; // Split the mysql_query
		for($i=1; $i<=$amt; $i++) {
...
// other_services delete case using checkboxes
if(isset($_POST['SubmitDelete'])) {
	$_POST = array_map("ucwords", $_POST);
	$amt = $_POST['total']; // Get the total rows
	for($i=1; $i<=$amt; $i++) {
		$delete = mysql_query("DELETE FROM other_services WHERE id = '".$_POST["delete$i"]."'"); // Run the delete query inside for loop
	}
...
// other_services update case
if(isset($_POST['SubmitUpdate'])) {
	$_POST = array_map("ucwords", $_POST);
	$amt = $_POST['total']; // Get the total rows
	for($i=1; $i<=$amt; $i++) {
		$update = mysql_query("UPDATE `other_services` SET `amt` = '".$_POST["amt$i"]."', cat = '".$_POST["cat$i"]."',
...
<meta name="keywords" content="other_services insert in php, other_services crud using PHP MySql, other_services insert update delete using php mysql"/>
<meta name="description" content="other_services insert update delete CRUD using PHP MySql. A simple way to insert, update and delete other_services values at a time using PHP MySql"/>
<style>
.as_wrapper{
	margin:0 auto;
	width:800px;
...
<div class="as_wrapper">
	<h1 class="h1"><a href="">Add/Edit and Delete Other services</a></h1>
    <p style="line-height:1.5">To add other_services note the Following<br>
   ...
   <span style="color:#f00"> 3. Please no CFA or Frs is allowed after figures</span><br>
    ...
    ?>
        ...
        <table align="center">
        <tr>
            <td style="background:#1188aa; color:#fff; padding:10px 10px">Number of other_services you want to add</td>
        ...
        ?>
            ...
            <tr>
                <td><?php echo $i; ?></td>
                <td><input type="text" name="amt<?php echo $i; ?>" style="width:300px"  />
               </td>                
