<?php
/**
Simple categories Create, Read, Update and Delete (CRUD) using php mysql by asif18.com, for more tutorials visit: http://www.asif18.com
for this tutorial visit: http://asif18.com/20/php/categories-insert-update-using-php-mysql-delete-categories-records-using-checkboxes-in-php/
**/
include '../dbc.php'; // include the database and server connection file
$query 	= mysql_query("SELECT * FROM `categories` ORDER BY id ASC"); // Select from the table
...
if(isset($_POST['submit'])) {
	...
	$amt = $_POST['total'];
	if($amt > 0) {
		$qry = "INSERT INTO categories(amt,facilities,cat,lastprice) VALUES "; // Split the mysql_query
		for($i=1; $i<=$amt; $i++) {
			$qry .= "('".$_POST["amt$i"]."', '".nl2br($_POST["facilities$i"])."','".$_POST["cat$i"]."', '".$_POST["lastprice$i"]."'), "; // loop the mysql_query values to avoid more server loding time
		}
...
// categories delete case using checkboxes
if(isset($_POST['SubmitDelete'])) {
	$_POST = array_map("ucwords", $_POST);
	$amt = $_POST['total']; // Get the total rows
	for($i=1; $i<=$amt; $i++) {
		$delete = mysql_query("DELETE FROM categories WHERE id = '".$_POST["delete$i"]."'"); // Run the delete query inside for loop
	}
...
// categories update case
if(isset($_POST['SubmitUpdate'])) {
	$_POST = array_map("ucwords", $_POST);
	$amt = $_POST['total']; // Get the total rows
	for($i=1; $i<=$amt; $i++) {
		$update = mysql_query("UPDATE `categories` SET `amt` = '".$_POST["amt$i"]."', cat = '".$_POST["cat$i"]."',
		 facilities = '".$_POST["facilities$i"]."',lastprice = '".$_POST["lastprice$i"]."' WHERE `id` = '".$_POST["id$i"]."'"); // Run the Mysql update query inside for loop
	}
...
<meta name="keywords" content="categories insert in php, categories crud using PHP MySql, categories insert update delete using php mysql"/>
<meta name="description" content="categories insert update delete CRUD using PHP MySql. A simple way to insert, update and delete categories values at a time using PHP MySql"/>
<style>
.as_wrapper{
	margin:0 auto;
	width:800px;
...
<div class="as_wrapper">
	<h1 class="h1"><a href="">Add/Edit and Delete a Category</a></h1>
    <p style="line-height:1.5">To add categories note the Following<br>
   ...
   <span style="color:#f00"> 3. Please no CFA or Frs is allowed after figures</span><br>
    ...
    ?>
        ...
        <table align="center">
        <tr>
            <td style="background:#1188aa; color:#fff; padding:10px 10px">Number of categories you want to add</td>
        ...
        ?>
            ...
            <tr style="background:#1188aa; color:#fff">
                ...
                <td align="center">Max Room Cost</td>
                <td align="center">Facilities </td>
                <td align="center">Category Name</td>
                <td align="center">Min Room Cost</td>
            ...
            <tr>
                <td><?php echo $i; ?></td>
                <td><input type="text" name="amt<?php echo $i; ?>" style="width:80px"  />
               </td>                
                    
                <td><textarea name="facilities<?php echo $i; ?>" cols="25" rows="8" />  </textarea>             
                </td>
                
                 <td><input type="text" name="cat<?php echo $i; ?>" />
               </td>                
                    
                <td><input type="text"name="lastprice<?php echo $i; ?>" style="width:120px" />               
                </td>
        ...
        elseif(isset($_GET['update'])) {
           ...
           <tr style="background:#1188aa; color:#fff">
            ...
            <tr>
                ...
                <td><input type="text" name="amt<?php echo $i; ?>" value="<?php echo $row['amt']; ?>" class="input" style="width:80px"  /></td>
                <td><input type="text" name="cat<?php echo $i; ?>" value="<?php echo $row['cat']; ?>" class="input"  /></td>
                
                 <td><textarea name="facilities<?php echo $i; ?>" cols="25" rows="8" class="input" />
           <?php echo $row['facilities']; ?>
                 
                 </textarea></td>
              <td><input type="text" name="lastprice<?php echo $i; ?>" value="<?php echo $row['lastprice']; ?>" class="input" style="width:80px" /></td>
