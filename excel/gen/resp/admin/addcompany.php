<?php
/**
Simple companies Create, Read, Update and Delete (CRUD) using php mysql by asif18.com, for more tutorials visit: http://www.asif18.com
for this tutorial visit: http://asif18.com/20/php/companies-insert-update-using-php-mysql-delete-companies-records-using-checkboxes-in-php/
**/
...
$query 	= mysql_query("SELECT * FROM `companies` ORDER BY id ASC"); // Select from the table
...
if(isset($_POST['submit'])) {
	...
	$amt = $_POST['total'];
	if($amt > 0) {
		$qry = "INSERT INTO companies(name) VALUES "; // Split the mysql_query
		for($i=1; $i<=$amt; $i++) {
...
// companies delete case using checkboxes
if(isset($_POST['SubmitDelete'])) {
	$_POST = array_map("ucwords", $_POST);
	$amt = $_POST['total']; // Get the total rows
	for($i=1; $i<=$amt; $i++) {
		$delete = mysql_query("DELETE FROM companies WHERE id = '".$_POST["delete$i"]."'"); // Run the delete query inside for loop
	}
...
// companies update case
if(isset($_POST['SubmitUpdate'])) {
	$_POST = array_map("ucwords", $_POST);
	$amt = $_POST['total']; // Get the total rows
	for($i=1; $i<=$amt; $i++) {
		$update = mysql_query("UPDATE `companies` SET  name = '".$_POST["name$i"]."' WHERE `id` = '".$_POST["id$i"]."'"); // Run the Mysql update query inside for loop
	}
...
<div class="as_wrapper">
	<h1 class="h1"><a href="">YOU ARE ADDING PARTNER COMPANIES</a></h1>
