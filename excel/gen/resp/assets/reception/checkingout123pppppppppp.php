...
while($rows=mysql_fetch_assoc($rusn)){
	 ...
	 elseif($diff<=$not) {
		  echo  $diff=$not;
                ...
                <tr>
                  <td>Amount Paid to you</td><td><input type="text" name="paid" style="width:300px" onBlur="doCalc(this.form)" required="required"  />
                </td></tr>

                
                <tr>
                  <td>Balance kept by us</td><td><input type="number" name="bal" style="width:300px; background:#ff6" required="required" readonly="readonly"   /></td></tr>
 ...
 <?php if(isset($_POST['saves'])){
	 ...
	 $update_finances=mysql_query("UPDATE finances set paid='".$bal."' ,bal='0',added='0', status='2',level='$date' where yourid='".$id."'") or die(mysql_error()); 
		//daily records
		$update_client=mysql_query("UPDATE ourclients set paid='".$bal."' ,bal='0' where yourid='".$id."'") or die(mysql_error());
		//update room status to empty
		
			$update_histo=mysql_query("UPDATE historique set paid='".$bal."' ,bal='0' where yourid='".$id."'") or die(mysql_error());
		//update room status to empty	
		
		$update_daily=mysql_query("UPDATE daily set rec='".$bal."'  where cust_id='".$id."'") or die(mysql_error());
		//update room status to empty	
		
		$update_rooms=mysql_query("UPDATE rooms set status='1' where num='".$room."'") or die(mysql_error());
		
		
	   $insert_report=mysql_query("INSERT INTO reports set checkin='$ind',date='$date',month='$month',year='$year',days_uesd='$added',withus='$bal',paidin='$paidold',paidout='$paid',cout='$date',name='$name',room='$room'") or die(mysql_error()) ;
