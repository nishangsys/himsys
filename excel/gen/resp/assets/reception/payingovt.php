...
?>
    ...
    <table>
          ...
          <tr><td></td><td><input type="hidden" name="id" value="<?php echo  $row['yourid']; ?>" style="width:300px" readonly="readonly" /></td></tr>
             ...
             <tr><td></td><td><input type="hidden" name="paidadd" value="<?php echo  $row['paid']; ?>"  /></td></tr>
...
while($rows=mysql_fetch_assoc($rusn)){
 ...
 <?php if(isset($_POST['saves'])){
		...
		if(($bal)<0){
			...
			//
			echo $update_finances=mysql_query("UPDATE finances set bal='".$bal."',status='2',paid='$paidad' where yourid='".$id."'") or die(mysql_error());
		//daily records
		$update_client=mysql_query("UPDATE ourclients set bal='".$bal."',paid='$paidad' where yourid='".$id."'") or die(mysql_error());
