...
$tale=$_GET['id'];
 ...
 $c=mysql_query("SELECT * FROM aorders where tab='$tale' and area='$a_area' and sector='$serial' and status='3'") or die(mysql_error());
   ...
   $result=mysql_query("SELECT product,category,SUM(qty),SUM(price),SUM(total),ids from ".$db_basket." where tab='".$tale."' and status='1'  group by product  ");
$O=1; 
		 ...
		 <div class="page">
          ...
          <h1 style="font-size:14px; text-align:center; font-family:Arial, Helvetica, sans-serif; margin-top:-5px"><?php echo $clients; ?></h1>
		   <h1 style="font-size:14px; text-align:center; font-family:Arial, Helvetica, sans-serif; margin-top:-5px">Tel:68 37 340 97/ 233 323 174</h1>
			 ...
			 <?php
			 $a=$con->query("SELECT sub from ".$db_basket." where tab='".$tale."' and status='1' and sub>0 GROUP BY tab") or die(mysqli_error($con));
...
<?php
$result=mysql_query("SELECT SUM(total) as total,tab from ".$db_basket." where tab='".$tale."' and status='1'   GROUP BY tab  ");
