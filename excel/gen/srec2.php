		 ...
		 <div class="page">
          ...
          <h1 style="font-size:14px; text-align:center; font-family:Arial, Helvetica, sans-serif; margin-top:-5px"><?php echo $clients; ?></h1>
		   <h1 style="font-size:14px; text-align:center; font-family:Arial, Helvetica, sans-serif; margin-top:-5px">Tel:679135426/</h1>
		     <h1 style="font-size:14px; text-align:center; font-family:Arial, Helvetica, sans-serif; margin-top:-5px">PAYMENT MODE: CASH</h1>


</div>
	   <?PHP
   $tale=$_GET['id'];
      $first=mysql_query("UPDATE ".$db_basket." set printed='2' where tab='".$tale."' and status='1'   ") or die(mysql_error());

   $result=mysql_query("SELECT product,category,SUM(qty),SUM(price),SUM(total),ids from ".$db_basket." where tab='".$tale."' and split='".$num."b' and status='1'  group by product  ");
$O=1; 
?>
...
<?php
$result=mysql_query("SELECT SUM(total) as total,tab from ".$db_basket." where tab='".$tale."' and split='".$num."b' and status='1'   GROUP BY tab  ");
