...
$duedate=date("m/d/Y", strtotime("$startDate +".($howlong-$af)." days"));
    ...
    $query 	= mysql_query("SELECT * FROM ".$db_basket." where tab='$who' and printed='0' and status= '3' ORDER BY id ASC") or die(mysql_error()); // Select from the table
  $totcount=mysql_num_rows($query);
 if($totcount>0){
	 echo "<script>alert('ERROR.Sorry Print a Receipt before Checking Client Out')</script>";
	 echo "<script>window.open('../401.php','_self')</script>";

	
 }
 else {

$query 	= mysql_query("SELECT * FROM ".$db_basket." where tab='$who' and status= '3' ORDER BY id ASC") or die(mysql_error()); // Select from the table
...
if(isset($_POST['SubmitUpdate'])) {
	...
	$update_tabs=mysql_query  ("UPDATE `".$db_table."` SET `status`= '2' WHERE num='$who' ") or die(mysql_error());
	
	
	
	 $update_split = mysql_query("UPDATE `splits` SET `status`= '2' WHERE sp='$who' and area='".$_GET['area']."' ") ;
			...
			rec='".$_POST["total$i"]."',date='".$_POST["date$i"]."',month='$month',year='$year',area='$section',time='$time',paidto='$paidto',purpose='bar/restau' ,idds='".$_POST["ids$i"]."',discount='".$_POST["discount$i"]."'") or die(mysql_error());
	...
	//historique insertion
		 ...
		 // finance insertion
			...
			paid='".$_POST["total$i"]."',area='".$_POST["ids$i"]."',date='".$_POST["date$i"]."',month='$month',year='$year'") or die(mysql_error());
			
		
		 $update_basket =mysql_query  ("UPDATE ".$db_basket." SET `status`= '2' WHERE tab='$who' and status= '3' ") or die(mysql_error());
		 
		  $update_basket12 =mysql_query  ("UPDATE `all_basket` SET `status`= '2' WHERE tab='$who' and status= '3' AND area='$a_area'") or die(mysql_error());
...
<h1 style="background:#088389; text-align:center; font-size:24px; padding:20px 0px; color:#fff;">Goods Validation for <span style="color:#ff0"> Table <?php echo $who; ?> </span></h1>
    ...
    ?>
            ...
            <tr style="background:#1188aa; color:#fff; height:30px; padding:10px 0px">
                ...
                <td align="center" class="tt">Discount</td>
            ...
            <tr>
                  ...
                  <td><input type="text" name="price<?php echo $i; ?>" value="<?php echo $row['price']; ?>" class="input" style="width:60px" readonly  /></td>
                   <td><input type="text" name="qty<?php echo $i; ?>" value="<?php echo $row['qty']; ?>" class="input" style="width:60px" readonly  /></td>
                     ...
                     <td><input type="text" name="total<?php echo $i; ?>" value="<?php echo $row['total']; ?>" class="input"  style="width:60px" readonly  /></td>
                      ...
                      <td><input type="text" name="discount<?php echo $i; ?>" value="<?php echo $row['sub']; ?>" class="input"  style="width:60px" readonly  /></td>
			   ...
			   $s=mysql_query("SELECT *  from products where product='".$row['product']."'
			    and serial='".$serial."' LIMIT 1 ") or die(mysql_error());
