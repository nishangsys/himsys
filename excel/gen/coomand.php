...
$query 	= mysql_query("SELECT * FROM ".$db_basket." where tab='$who' and status='1'   ORDER BY id ASC") or die(mysql_error()); // Select from the table
...
if(isset($_POST['SubmitUpdate'])) {
	...
	//daily records
		 ...
		 $update_basket =mysql_query  ("UPDATE ".$db_basket." SET `status`= '3' WHERE tab='$who' and status='1' ") or die(mysql_error());
		 
		  $update_basket12 =mysql_query  ("UPDATE `all_basket` SET `status`= '3' WHERE tab='$who' and status='1' AND area='".$_GET['area']."' ") or die(mysql_error());
...
<h1 style="background:#088389; text-align:center; font-size:24px; padding:20px 0px; color:#fff;">VALIDATING <span style="color:#ff0"> Table <?php echo $who; ?> </span>COMMAND OR ROUND </h1>
    ...
    ?>
            ...
            <tr>
                ...
                <input type="submit" name="SubmitUpdate" value="Valid Sales Command or Round" /></td>
            </tr>
            </table>
            </form>
