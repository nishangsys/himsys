...
<?PHP
 ...
 <a href="?go=<?php echo $row['name']; ?>&table=<?php echo $_SESSION['table']; ; ?><?php echo $_GET['table']; ; ?>&sec=<?php echo $_GET['area']; ?><?php echo $_GET['sec']; ?>" style="color:#fff; font-size:18px; text-decoration:none"> <i class="fa fa-home fa-5x"></i> <?php echo $row['name']; ?> </a>
...
//VIP Bar or Night Club
 ...
 ?>
    ...
    <br />
    <h1 class="he" style="margin-top:-10px; text-align:center"><?php echo $comp; ?>  Bills Today <span style="color:#Ff0"></span></h1>
   ...
   <div class="clear"></div><br />
    ...
    <?php
	 ...
	 $ress=mysql_query("SELECT product,category,tab,time,SUM(qty),price,SUM(total),id,section from ".$db_basket." where  section=".$serial." and  tab='$tabl' and status!='2' and date='$today' group by id order by id DESC   ");
$num=1;
	?>
      
       <table style="line-height:1.9; margin-top:-30px">
				<tr style="background:#1188aa; color:#fff; padding:10px 10px"><td>S/N</td><td>Item</td><td>Company Name</td><td>Price</td><td>Qty</td><td>TC</td><td>Time</td><td>Back to Stock</td></tr>
        ...
        <tr>
          ...
          <td><a href="delall12.php?hist_id=<?php echo $getres['id']; ?>&section=<?php echo $getres['section']; ?> &comp=<?php echo $_GET['go'];  ?>&table=<?php echo $_GET['table'];  ?>" onClick="return confirm('Are you Sure you want to Delete  <?php echo $getres['product']; ?>?');" >
        Back to Stock</a></td>|
        ...
        <?php } ?>
         ...
         <tr>
          ...
          <?PHP $mk=mysql_query("SELECT  SUM(total) as totals from ".$db_basket." where  tab='$tabl' and section=".$serial." and  status!='2' and date='$today' group by ids  ");
...
if(isset($_POST['date'])){
	...
	$r=mysql_query("SELECT product,category,SUM(qty),SUM(profit),price,SUM(total),id,tab,section from ".$db_basket." where tab='$tabl' and section=".$serial."   and status!='2'  and date='$today' group by tab ");
	...
	//daily records
	$daily=mysql_query("INSERT INTO daily set cust_id='".$serial."',room='company',amt='".$profi."',reason='staff ".$serial." bills',total='$totbill',
			rec='$totpaid',date='$date',month='$month',year='$year',area='".$section."',time='$time',paidto='$paidto',owed='$bal',purpose='".$serial."  bills' ,idds='".$section."',staffname='$compname',qty='$qtyy'") or die(mysql_error());
			
			
		
	 $update_split = mysql_query("UPDATE `splits` SET `status`= '2' WHERE sp='$tabl' and area='".$_GET['sec']."' ") ;	
			
		  $update_basket12 =mysql_query  ("UPDATE ".$db_basket." SET `status`= '2',tab='$comp' WHERE tab='$tabl'  and status!='2'    ") or die(mysql_error());
		  
		   $ol=mysql_query("UPDATE ".$db_table." SET status='2' where num='$tabl' ") or die(mysql_error());
