...
<body>
    ...
    <h1 class="he">Free Distributed Products<span style="color:#Ff0"><?php echo date('F d-m-Y'); ?> </span></h1>
   
     <?php 
	 $tab=$_GET['id'];  	   
$ress=mysql_query("SELECT product,category,SUM(qty),price,SUM(total),id,section from ".$db_basket." where tab='".$tab."' and status!='2' group by id order by id DESC   ");
$num=1;
       ...
       <table style="line-height:1.9">
        ...
        <?php } ?>
         ...
         <tr>
          ...
          <?PHP $mk=mysql_query("SELECT  SUM(total) as totals from ".$db_basket." where tab='".$tab."' and status!='2' and date='$today' group by tab  ");
    ...
    <a href="<?php $_SEVER['_SELF']; ?>?date=<?php echo date('d-m-Y'); ?>&area=<?php echo $section; ?>&tabs=<?php echo $tab; ?>" style="background:#1188aa; color:#fff; padding:10px 10px; margin-top:20px">Summarize and Leave</a>
...
if($section=='20' or $section=='18'){
	...
	$r=mysql_query("SELECT product,category,SUM(qty),SUM(profit),price,SUM(total),id,section from ".$db_basket." where tab='".$otabs."' and status!='2'  group by tab  ");
	...
	//daily records
	$daily=mysql_query("INSERT INTO daily set cust_id='bar',room='free',amt='0',reason='".$prod."',
			rec='free',date='$date',month='$month',year='$year',area='".$section."',time='$time',paidto='$paidto',purpose='".$serial."' ,idds='".$section."'") or die(mysql_error()) ;
			
			
		  $update_basket12 =mysql_query  ("UPDATE ".$db_basket." SET `status`= '2',tab='free' WHERE tab='".$otabs."' and status!='2'  ") or die(mysql_error());
