 ...
 <form class="form-horizontal" action="" method="post" name="form">
  ...
  <input name="username" type="text" value="<?php echo $rows['student_name'];; ?>" id="username" class="demoInputBox" onBlur="checkAvailability()" style="width:65%; border:2px solid#f00" required="required"><span id="user-availability-status" ></span>    
    ...
    <div class="form-group">
      ...
      <div class="col-sm-10">
        <input type="number" class="form-control" id="pwd" name="part" value="<?PHP 	 
 echo $rows['amount_paid']; ?>" onBlur="doCalc(this.form)" >
      </div>
    </div>
    ...
    <div class="form-group">
      ...
      <div class="col-sm-10">
        <input type="number" class="form-control" id="pwd" name="balance"  value="<?php
	 echo $rows['balance'];
						 
				  ?>"  required="required" style="background:#FFC; color:#000; font-family:'Arial Black', Gadget, sans-serif">
      </div>
    </div>
    ...
    <div class="form-group">
      <div class="col-sm-offset-2 col-sm-10">
        <button type="submit" class="btn btn-primary" name="save" class="btn btn-danger">Save UPDATE</button>
      </div>
    </div>
  </form>
...
class='$class1',amountpaid='$disc',amount_paid='$part',expected_amount='$feeamt',balance='$bal',date='$date' ,year_id='$year_id',photo='$matric' ,time='$levels',olddebt='$reg' WHERE roll='".$_GET['fin']."' " ) or die(mysqli_error($conn));
 ...
 $query5545=$con->query("UPDATE daily  set company='$banks' where year='$current' and cust_id='$matriculex'" ) or die(mysqli_error($con));
